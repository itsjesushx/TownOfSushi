...
public sealed class LoverModifier : AllianceGameModifier, IWikiDiscoverable, IAssignableTargets
{
    ...
    {
        var knowsRole = OptionGroupSingleton<LoversOptions>.Instance.KnowEachOthersRole &&
        PlayerControl.LocalPlayer.HasModifier<LoverModifier>() && player.HasModifier<LoverModifier>();

        return knowsRole;
    }

    public override bool DoesTasks =>
        (OtherLover == null || OtherLover.Is(Factions.Crewmate)) &&
        Player.Is(Factions.Crewmate); // Lovers do tasks if they are not lovers with an Evil

    public override bool HideOnUi => false;
    public override LoadableAsset<Sprite>? ModifierIcon => TownOfSushiAssets.Lover;

    public PlayerControl? OtherLover { get; set; }

    public override int CustomAmount =>
        (int)OptionGroupSingleton<LoversOptions>.Instance.LoversChance != 0 ? 2 : 0;

    public override int CustomChance => (int)OptionGroupSingleton<LoversOptions>.Instance.LoversChance;
    ...
    public void AssignTargets()
    {
        ...
        if (chance <= (int)OptionGroupSingleton<LoversOptions>.Instance.LoversChance)
        {
            var loveOpt = OptionGroupSingleton<LoversOptions>.Instance;
            var impTargetPercent = (int)loveOpt.LovingImpPercent;

            var players = PlayerControl.AllPlayerControls.ToArray()
                .Where(x => !x.HasDied() && !x.HasModifier<ExecutionerTargetModifier>() && !x.HasModifier<AllianceGameModifier>() && (loveOpt.NeutralLovers || !x.IsPassiveNeutral())).ToList();
            players.Shuffle();
            ...
            foreach (var player in players.SelectMany(_ => players))
            {
                ...
                else if (player.Is(Factions.Crewmate) ||
