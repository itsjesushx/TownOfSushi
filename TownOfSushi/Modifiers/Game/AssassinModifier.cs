...
public abstract class AssassinModifier : ExcludedGameModifier
{
    ...
    public void ClickGuess(PlayerVoteArea voteArea, MeetingHud meetingHud)
    {
        ...
        void ClickModifierHandle(BaseModifier modifier)
        {
            ...
            LastAttemptedVictim = player;
            LastGuessedItem =
                $"{Utils.GetRoleColour(modifier.ModifierName.Replace(" ", string.Empty)).ToTextColor()}{modifier.ModifierName}</color>";
        }
        ...
        void ClickHandler(PlayerControl victim)
        {
            ...
            if (victim == Player && Player.TryGetModifier<DoubleShotModifier>(out var modifier) && !modifier.Used)
            {
                ...
                Coroutines.Start(Utils.CoFlash(TownOfSushiColors.Impostor));

                var notif1 = Helpers.CreateAndShowNotification(
                    Utils.ColorString(TownOfSushiColors.ImpSoft, $"<b>Your Double Shot has prevented you from dying this meeting!</b>"),
                    Color.white, spr: TownOfSushiAssets.DoubleShot.LoadAsset());
            ...
            if (victim.IsProtected() && OptionGroupSingleton<AssassinOptions>.Instance.CantGuessProtectedPlayer)
            {
                ...
                var notif1 = Helpers.CreateAndShowNotification(
                    Utils.ColorString(TownOfSushiColors.ImpSoft, $"<b>{victim.cosmetics.nameText} is Protected. You cannot guess them until their protector dies!</b>"),
                    Color.white, spr: TownOfSushiAssets.Medic.LoadAsset());
                notif1.AdjustNotification();
    ...
    public bool IsExempt(PlayerVoteArea voteArea)
    {
        ...
        return voteArea?.TargetPlayerId == Player.PlayerId ||
               Player.Data.IsDead ||
               voteArea!.AmDead ||
               Player.IsImpostor() && votePlayer?.IsImpostor() == true && !Utils.SpyInGame() ||
    ...
    private bool IsRoleValid(RoleBehaviour role)
    {
        ...
        var touRole = role as ITOSRole;
        var assassinRole = Player.Data.Role as ITOSRole;
        ...
        if (role.IsImpostor() &&
            assassinRole?.RoleAlignment is RoleAlignment.NeutralKilling or RoleAlignment.CrewmateInvestigative 
            or RoleAlignment.CrewmateKilling or RoleAlignment.CrewmatePower 
            or RoleAlignment.CrewmateProtective or RoleAlignment.CrewmateSupport)
        {
            return true;
        }
