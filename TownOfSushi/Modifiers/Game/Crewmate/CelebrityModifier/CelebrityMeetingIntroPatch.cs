...
public static class CelebrityMeetingIntroPatch
{
    ...
    public static void MeetingIntroAnimationPatch(MeetingIntroAnimation __instance)
    {
        ...
        iconSprite.sprite = TownOfSushiAssets.Celebrity.LoadAsset();

        if (HudManager.Instance != null)
        {
            var title = $"<color=#{TownOfSushiColors.Celebrity.ToHtmlStringRGBA()}>Celebrity Report</color>";
            Utils.AddFakeChat(celebrity.Player.Data, title, celebrity.DeathMessage, false, true);
        }
    }
}