...
public static class SwapperEvents
{
    ...
    public static void ProcessVotesEventHandler(ProcessVotesEvent @event)
    {
        ...
        ModifierUtils.GetActiveModifiers<SwapperModifier>().Do(x => SwapVotes(@event, x));
    }

    [RegisterEvent]
    public static void VotingCompleteEventHandler(VotingCompleteEvent @event)
    {
        ModifierUtils.GetActiveModifiers<SwapperModifier>().Do(x => x.OnVotingComplete());

        if (!ModifierUtils.GetActiveModifiers<SwapperModifier>().Any())
        {
            return;
        }
    ...
    private static void SwapVotes(ProcessVotesEvent @event, SwapperModifier swapper)
    {
        if (swapper == null || swapper.Player.HasDied() || !swapper.Swap1 || !swapper.Swap2)
        {
            return;
        }
    ...
    {
        var SwapperModifiers = ModifierUtils.GetActiveModifiers<SwapperModifier>().ToList();

        var duration = 4f / (SwapperModifiers.Count + 1);

        foreach (var role in SwapperModifiers)
        {
    ...
    {
        yield return Utils.PerformTimedAction(duration, p => target.position = Vector3.Lerp(source, dest, p));
    }
}