    ...
    public void OnRoundStart()
    {
        if (Player.AmOwner && Player.myTasks.Count > 0 && !Player.HasDied())
        {
            ...
            if (tasks.Count > 0)
            {
                ...
                var notif1 = Helpers.CreateAndShowNotification($"<b>{TownOfSushiColors.Taskmaster.ToTextColor()}The task '{taskText}' has been completed for you.</b></color>", Color.white, spr: TosModifierIcons.Taskmaster.LoadAsset());
