...
public class DoubleShotModifier : TOSGameModifier, IWikiDiscoverable
{
    ...
    public override LoadableAsset<Sprite>? ModifierIcon => TownOfSushiAssets.DoubleShot;
    ...
    public override int CustomAmount =>
        (int)OptionGroupSingleton<ImpostorDoubleShotOptions>.Instance.ImpostorDoubleShotAmount + (int)OptionGroupSingleton<NeutralRolesOptions>.Instance.DoubleShotAmount;

    public override int CustomChance
    {
        get
        {
            var neutOpt = OptionGroupSingleton<NeutralRolesOptions>.Instance;
