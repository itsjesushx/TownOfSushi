...
public static class Utils
{
    ...
    public static bool TwoPlayersAlive() => PlayerControl.AllPlayerControls.ToArray().Count(x => !x.HasDied()) <= 2;
    ...
    public static Factions GetFactions(this ICustomRole role)
    {
        if (role is ITOSRole touRole)
        {
            return touRole.Faction;
        }
        ...
        {
            return Factions.Impostor;
        }
        else
        {
            return Factions.Crewmate;
        }
    }
    public static RoleAlignment GetRoleAlignment(this ICustomRole role)
    {
        ...
        if (basicRole!.IsNeutral())
        {
            return RoleAlignment.NeutralBenign;
        }
        else if (basicRole!.IsImpostor())
        {
    ...
    public static string GetParsedFaction(Factions faction)
    {
        var factionName = $"{faction.ToDisplayString()}";
        var text = factionName;

        if (text.Contains("Crewmate"))
        {
            text = $"<color=#68ACF4>{text}</color>";
        }
        else if (text.Contains("Impostor"))
        {
            text = $"<color=#D63F42>{text}</color>";
        }
        else
        {
            text = $"<color=#8A8A8A>{text}</color>";
        }
        return text;
    }
    ...
    public static string AppendOptionsText(Type classType)
    {
        ...
        builder.AppendLine(ColorString(TownOfSushiColors.Sheriff, $"\n<size=50%> \n</size><b>Options</b>"));
    ...
    {
        var roles = AllRoles.Where(x => x is ITOSRole role && role.Team == team);
        var registeredRoles = roles.ToList();
    ...
    public static IEnumerable<RoleBehaviour> GetRoles(RoleAlignment alignment)
    {
        return CustomRoleUtils.GetActiveRoles()
            .Where(x => x is ITOSRole role && role.RoleAlignment == alignment);
    }
    ...
    private static IEnumerator BouncePrivateChatDot(BubbleType bubbleType)
    {
        ...
        {
            BubbleType.None => TownOfSushiAssets.NormalBubble.LoadAsset(),
            BubbleType.Impostor => TownOfSushiAssets.ImpBubble.LoadAsset(),
            BubbleType.Vampire => TownOfSushiAssets.VampBubble.LoadAsset(),
            BubbleType.Lover => TownOfSushiAssets.LoveBubble.LoadAsset(),
            BubbleType.Jailor => TownOfSushiAssets.JailBubble.LoadAsset(),
            BubbleType.Lawyer => TownOfSushiAssets.Lawyer.LoadAsset(),
            _ => null,
        };
    ...
    public static void RpcHostSuicide(PlayerControl player)
    {
        ...
        Coroutines.Start(CoFlash(TownOfSushiColors.ImpSoft, PlaySound: true));
    ...
    public static GameObject CreateSpherePrimitive(Vector3 location, float radius)
    {
        ...
        spherePrimitive.GetComponent<MeshRenderer>().material = AnimationMaterials.BombMaterial.LoadAsset();
    ...
    public static ArrowBehaviour CreateArrow(Transform parent, Color color)
    {
        ...
        renderer.sprite = TownOfSushiAssets.ArrowSprite.LoadAsset();
    ...
    public static (ushort RoleType, int Chance) SelectRole(List<(ushort RoleType, int Chance)> roles)
    {
        ...
        var random2 = Random.RandomRangeInt(1, total + 1);
