...
{
    public static ITOSRole? GetTownOfSushiRole(this PlayerControl player)
    {
        var role = player.Data?.Role as ITOSRole;
    ...
    public static bool Is(this PlayerControl player, Factions faction)
    {
        if (player.Data.Role is ITOSRole role && role.Faction == faction)
        {
            return true;
        }

        if ((player.Data.Role.TeamType is RoleTeamTypes.Crewmate) && faction == Factions.Crewmate)
        {
            return true;
        }
    
        if ((player.Data.Role.TeamType is RoleTeamTypes.Impostor) && faction == Factions.Impostor)
        {
            return true;
        }
    ...
    {
        if (player.Data.Role is ITOSRole role && role.RoleAlignment == roleAlignment)
        {
            return true;
        }

        if ((player.Data.Role.Role is RoleTypes.Crewmate or RoleTypes.Scientist
        or RoleTypes.Noisemaker or RoleTypes.Engineer)
        ...
        if ((player.Data.Role.Role is RoleTypes.Shapeshifter or RoleTypes.Phantom) && roleAlignment == RoleAlignment.ImpostorConcealing)
        {
            return true;
        }
    ...
    public static IEnumerator CoClean(this DeadBody body)
    {
        ...
        yield return Utils.PerformTimedAction(1f, t => renderer.color = renderer.color.SetAlpha(1 - t));
    ...
    public static void SetRole(this ShapeshifterPanel panel, int index, RoleBehaviour roleBehaviour, Action onClick)
    {
        ...
        var roleImg = TownOfSushiAssets.RandomImp.LoadAsset();
        ...
        //var teamName = roleBehaviour is ITOSRole touRole
        //    ? touRole.Alignment.ToDisplayString()
        //    : roleBehaviour.TeamType.ToDisplayString();
        //if (roleBehaviour is ICustomRole customOther && roleBehaviour is not ITOSRole) teamName = customOther.Team.ToDisplayString();
        ...
        //else if (roleBehaviour is not ITOSRole)
        //{
        //    if (roleBehaviour is ITOSRole) teamName = "Neutral Benign";
        //    else if (roleBehaviour is ITOSRole) teamName = "Neutral Evil";
        //    else if (roleBehaviour is ITOSRole) teamName = "Neutral Killing";
        //    teamName = teamName.Replace("Neutral", $"<color=#8A8A8AFF>Neutral</color>");
        //}

        var alignment = roleBehaviour is ITOSRole touRole
    ...
    public static void SetModifier(this ShapeshifterPanel panel, int index, BaseModifier modifier, Action onClick)
    {
        ...
        var modImg = TownOfSushiAssets.RandomAny.LoadAsset();
        ...
        if (modifier is TOSGameModifier touModifier)
        {
            ...
            {
                modImg = TownOfSushiAssets.RandomCrew.LoadAsset();
            }
            else if (touModifier.FactionType.ToDisplayString().Contains("Neutral"))
            {
                modImg = TownOfSushiAssets.RandomNeut.LoadAsset();
            }
            else if (touModifier.FactionType.ToDisplayString().Contains("Impostor"))
            {
                modImg = TownOfSushiAssets.RandomImp.LoadAsset();
            }
    ...
    [MethodRpc((uint)TownOfSushiRpc.ChangeRole, SendImmediately = true)]
    ...
    [MethodRpc((uint)TownOfSushiRpc.PlayerExile, SendImmediately = true)]
    public static void RpcPlayerExile(this PlayerControl player)
    {
        player.Exiled();
    }

    [MethodRpc((uint)TownOfSushiRpc.SetPos, SendImmediately = true)]
    ...
    public static void GhostFade(this PlayerControl player)
    {
        ...
        if (player.GetAppearanceType() != TownOfSushiAppearances.PlayerOnly)
        {
            var fade = new VisualAppearance(player.GetDefaultModifiedAppearance(), TownOfSushiAppearances.PlayerOnly)
            {
                HatId = string.Empty,
                SkinId = string.Empty,
    ...
    public static GameObject CreateHackedIcon(this ActionButton button)
    {
        ...
        render.sprite = TownOfSushiAssets.Hacked.LoadAsset();
