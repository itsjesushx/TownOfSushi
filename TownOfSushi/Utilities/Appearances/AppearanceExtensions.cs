...
public static class AppearanceExtensions
{
    public static void ResetAppearance(this PlayerControl player, bool override_checks = false, bool fullReset = false)
    {
        ...
        if (player.GetAppearanceType() == TownOfSushiAppearances.Paint && !override_checks)
        {
            return;
        }
        if (fullReset)
    ...
    {
        if (toggle && player.GetAppearanceType() != TownOfSushiAppearances.Camouflage && !player.HasDied())
        {
    ...
    public static VisualAppearance GetDefaultModifiedAppearance(this PlayerControl playerControl)
    {
        ...
        if (playerControl.MyPhysics.bodyType is PlayerBodyTypes.Horse or PlayerBodyTypes.LongSeeker)
        {
            appearance = new VisualAppearance(playerControl.Data.DefaultOutfit, TownOfSushiAppearances.Default)
            {
                SkinId = string.Empty
            };
        }
