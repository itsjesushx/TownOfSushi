...
public static class PlayerRoleTextExtensions
{
    ...
    public static string UpdateTargetSymbols(this string name, PlayerControl player, bool hidden = false)
    {
        ...
        if ((player.HasModifier<LawyerClientModifier>(x => x.OwnerId == PlayerControl.LocalPlayer.PlayerId) &&
             PlayerControl.LocalPlayer.IsRole<LawyerRole>())
            || (player.HasModifier<LawyerClientModifier>() && PlayerControl.LocalPlayer.HasDied() &&
                genOpt.TheDeadKnow && !hidden))
        {
            name += "<color=#70b98d> $</color>";
        }
