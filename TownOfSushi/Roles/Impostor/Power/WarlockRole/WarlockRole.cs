...
public sealed class WarlockRole(IntPtr cppPtr)
    : ImpostorRole(cppPtr), ITOSRole, IWikiDiscoverable, ICrewVariant, IMysticClue
{
    ...
    public void FixedUpdate()
    {
        ...
        HudManager.Instance.KillButton.ToggleVisible(
            ...
            (Player != null && Utils.ImpAliveCount == 1));
    }

    public CustomRoleConfiguration Configuration => new(this)
    {
        UseVanillaKillButton = true,
        IntroSound = TownOfSushiAudio.WitchIntro,
        Icon = TownOfSushiAssets.Warlock
    };
    ...
    public static void RpcCurseKill(PlayerControl source, PlayerControl target, PlayerControl player)
    {
        ...
        if (target.HasModifier<CrusaderFortifiedModifier>())
        {
            ...
            if (player.AmOwner)
            {
                var notif2 = Helpers.CreateAndShowNotification(Utils.ColorString(TownOfSushiColors.Crusader,
                $"<b>{source.Data.PlayerName} has tried to kill {target.Data.PlayerName}, but they are fortified! Your murder attempt backfired.</b>"),
                Color.white, spr: TownOfSushiAssets.Crusader.LoadAsset());
        ...
        if (player.AmOwner)
        {
            var notif3 = Helpers.CreateAndShowNotification(Utils.ColorString(TownOfSushiColors.ImpSoft,
            $"<b>{source.Data.PlayerName} has killed {target.Data.PlayerName}.</b>"),
            Color.white, spr: TownOfSushiAssets.Warlock.LoadAsset());
    ...
    [
        new("Curse",
            ...
            TownOfSushiAssets.WarlockCurseButton)
    ];
}