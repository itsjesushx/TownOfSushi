...
public sealed class SwoopModifier : ConcealedModifier, IVisualAppearance
{
    ...
    {
        var playerColor = PlayerControl.LocalPlayer.IsImpostor() ? new Color(0f, 0f, 0f, 0.1f) : Color.clear;
    ...
    public override void OnActivate()
    {
        if (Player.AmOwner)
        {
            TownOfSushiAudio.PlaySound(TownOfSushiAudio.SwooperActivateSound);
        }
        ...
        button.OverrideSprite(TownOfSushiAssets.UnswoopSprite.LoadAsset());
    ...
    public override void OnDeactivate()
    {
        ...
        var button = CustomButtonSingleton<SwooperSwoopButton>.Instance;
        button.OverrideSprite(TownOfSushiAssets.SwoopSprite.LoadAsset());
        button.OverrideName("Swoop");

        if (Player.AmOwner)
        {
            TownOfSushiAudio.PlaySound(TownOfSushiAudio.SwooperDeactivateSound);
        }
