...
public sealed class EclipsalBlindModifier(PlayerControl player) : DisabledModifier
{
    ...
    public override void OnActivate()
    {
        ...
        EclipseBack = AnimStore.SpawnAnimBody(Player, TownOfSushiAssets.EclipsedPrefab.LoadAsset(), false, -1.1f)!;
        EclipseBack.SetActive(false);
        if (Player.AmOwner && !Eclipsal.AmOwner)
        {
            var notif1 = Helpers.CreateAndShowNotification(Utils.ColorString(TownOfSushiColors.Eclipsal,
                $"<b>You were blinded by an Eclipsal!</b>"), Color.white,
                spr: TownOfSushiAssets.Eclipsal.LoadAsset());
