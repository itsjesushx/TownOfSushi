...
public sealed class WerewolfRole(IntPtr cppPtr) : NeutralRole(cppPtr), ITOSRole, IWikiDiscoverable, ICrewVariant, IMysticClue
{
    ...
    public CustomRoleConfiguration Configuration => new(this)
    {
        ...
        IntroSound = TownOfSushiAudio.PredatorTerminateSound,
        Icon = TownOfSushiAssets.Werewolf,
    ...
    public override void Initialize(PlayerControl player)
    {
        ...
        if (Player.AmOwner)
        {
            HudManager.Instance.ImpostorVentButton.graphic.sprite = TownOfSushiAssets.PredatorVentSprite.LoadAsset();
    ...
    public static void RpcMaulMurder(PlayerControl player)
    {
        ...
        var nearbyPlayers = Utils.GetClosestPlayers(role.Player.GetTruePosition(), OptionGroupSingleton<WerewolfOptions>.Instance.RaulRadius);
    ...
    {
        return "The Werewolf is a Neutral Killing role that wins by being the last killer alive. When they kill someone using their Maul button, any nearby player will die. The range between the players that can die and the Werewolf can be changed in settings." + Utils.AppendOptionsText(GetType());
    }
