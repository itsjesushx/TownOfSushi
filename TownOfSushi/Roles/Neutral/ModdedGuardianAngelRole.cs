...
public sealed class ModdedGuardianAngelRole(IntPtr cppPtr) : NeutralRole(cppPtr), ITownOfSushiRole, IWikiDiscoverable, IDoomable, IAssignableTargets, ICrewVariant
{
    ...
    private static IEnumerator SetTutorialTargets(ModdedGuardianAngelRole ga)
    {
    ...
    public static bool GASeesRoleVisibilityFlag(PlayerControl player)
    {
        var gaKnowsTargetRole = OptionGroupSingleton<GuardianAngelOptions>.Instance.GAKnowsTargetRole &&
            PlayerControl.LocalPlayer.IsRole<ModdedGuardianAngelRole>() &&
            PlayerControl.LocalPlayer.GetRole<ModdedGuardianAngelRole>()!.Target == player;

        return gaKnowsTargetRole;
    }
    public static bool GATargetSeesVisibilityFlag(PlayerControl player)
    {
        var gaTargetKnows = OptionGroupSingleton<GuardianAngelOptions>.Instance.ShowProtect is ProtectOptions.SelfAndGA &&
            player.HasModifier<GuardianAngelTargetModifier>();

        var gaKnowsTargetRole = PlayerControl.LocalPlayer.IsRole<ModdedGuardianAngelRole>() &&
            PlayerControl.LocalPlayer.GetRole<ModdedGuardianAngelRole>()!.Target == player;
    ...
    public void AssignTargets()
    {
        ...
        var gas = PlayerControl.AllPlayerControls.ToArray()
            .Where(x => x.IsRole<ModdedGuardianAngelRole>() && !x.HasDied());
    ...
    {
        if (player.Data.Role is not ModdedGuardianAngelRole)
        {
        ...
        var role = player.GetRole<ModdedGuardianAngelRole>();
