    ...
    : NeutralRole(cppPtr), ITOSRole, IWikiDiscoverable, ICrewVariant, IGuessable, IMysticClue
{
    ...
    {
        IntroSound = TownOfSushiAudio.MediumIntroSound,
        GhostRole = (RoleTypes)RoleId.Get<NeutralGhostRole>(),
        Icon = TownOfSushiAssets.Amnesiac
    };
    ...
    [
        new("Remember",
            ...
            TownOfSushiAssets.RememberButtonSprite),
        new("Vest",
            "Put on a Vest protecting you from attacks.",
            TownOfSushiAssets.VestSprite)
    ];
    ...
    public static void RpcRemember(PlayerControl player, PlayerControl target)
    {
        ...
        if (roleWhenAlive is AmnesiacRole)
        {
            if (player.AmOwner)
            {
                var notif1 = Helpers.CreateAndShowNotification(
                    $"<b>{target.CachedPlayerData.PlayerName} was an " + Utils.ColorString(TownOfSushiColors.Amnesiac, $"<b>Amnesiac</b>") + ", so their role cannot be picked up.</b>",
                    Color.white, new Vector3(0f, 1f, -20f), spr: TownOfSushiAssets.Amnesiac.LoadAsset());
        ...
        {
            var notif1 = Helpers.CreateAndShowNotification(
                Utils.ColorString(TownOfSushiColors.Amnesiac, $"<b>The Amnesiac has remembered a role from the death!</color>.</b>"),
                Color.white, new Vector3(0f, 1f, -20f), spr: TownOfSushiAssets.Amnesiac.LoadAsset());
            notif1.AdjustNotification();
        }
        ...
        else if (player.Is(RoleAlignment.NeutralKilling) && OptionGroupSingleton<AssassinOptions>.Instance.AmneTurnNeutAssassin)
        {
            player.AddModifier<NeutralKillerAssassinModifier>();
        }
        else if (player.Is(Factions.Crewmate) && OptionGroupSingleton<AssassinOptions>.Instance.AmneTurnCrewAssassin)
        {
