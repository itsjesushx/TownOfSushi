...
public sealed class InquisitorRole(IntPtr cppPtr) : NeutralRole(cppPtr), ITownOfSushiRole, IWikiDiscoverable, IDoomable, IAssignableTargets, ICrewVariant
{
    ...
    public Color RoleColor => TownOfSushiColors.Inquisitor;
    ...
    {
        Logger<TownOfSushiPlugin>.Info($"Generating Inquisitor report");
        ...
        foreach (var player in GameData.Instance.AllPlayers.ToArray().Where(x => !x.Object.HasDied() && x.Object.HasModifier<InquisitorInquiredModifier>()))
        {
            ...
            {
                reportBuilder.AppendLine(TownOfSushiPlugin.Culture, $"Your inquiry reveals that {player.PlayerName} is a heretic!\n");
                ...
                reportBuilder.AppendLine(TownOfSushiPlugin.Culture, $"Your inquiry reveals that {player.PlayerName} is not a heretic!");
        ...
        {
            var title = $"<color=#{TownOfSushiColors.Inquisitor.ToHtmlStringRGBA()}>Inquisitor Report</color>";
    ...
    public void AssignTargets()
    {
        ...
        if (inquis == null)
        {
            Logger<TownOfSushiPlugin>.Error("Inquisitor not found.");
            return;
        }
        ...
        // Logger<TownOfSushiPlugin>.Warning($"Players in heretic list possible: {players.Count}");
        players.Shuffle();
        players.Shuffle();
        players.Shuffle();
    ...
    [MethodRpc((uint)TownOfSushiRpc.AddInquisTarget, SendImmediately = true)]
    public static void RpcAddInquisTarget(PlayerControl player, PlayerControl target)
    {
        if (player.Data.Role is not InquisitorRole)
        {
            Logger<TownOfSushiPlugin>.Error("RpcAddInquisTarget - Invalid Inquisitor");
            return;
        }
    ...
    [MethodRpc((uint)TownOfSushiRpc.InquisitorWin, SendImmediately = true)]
    public static void RpcInquisitorWin(PlayerControl player)
    {
        if (player.Data.Role is not InquisitorRole)
        {
            Logger<TownOfSushiPlugin>.Error("RpcInquisitorWin - Invalid Inquisitor");
            return;
        }
    ...
    public List<CustomButtonWikiDescription> Abilities { get; } = [
        ...
        new("Vanquish",
            ...
            TosNeutAssets.InquisKillSprite)
    ];
}
