    ...
    public void CheckTaskRequirements()
    {
        ...
        if (!Revealed && tasksRemaining == (int)OptionGroupSingleton<HaunterOptions>.Instance.NumTasksLeftBeforeAlerted)
        {
            ...
            if (Player.AmOwner)
            {
                ...
                var notif1 = Helpers.CreateAndShowNotification($"<b>{TownOfSushiColors.Haunter.ToTextColor()}You have alerted the Killers!</b></color>", Color.white, new Vector3(0f, 1f, -20f), spr: TosRoleIcons.Haunter.LoadAsset());
            ...
            else if (IsTargetOfHaunter(PlayerControl.LocalPlayer))
            {
                ...
                var notif1 = Helpers.CreateAndShowNotification($"<b>{TownOfSushiColors.Haunter.ToTextColor()}A Haunter is loose, catch them before they reveal you!</b></color>", Color.white, new Vector3(0f, 1f, -20f), spr: TosRoleIcons.Haunter.LoadAsset());
                notif1.Text.SetOutlineThickness(0.35f);
            }
        }
    }
