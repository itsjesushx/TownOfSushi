...
public sealed class TransporterRole(IntPtr cppPtr) : CrewmateRole(cppPtr), ITownOfSushiRole, IWikiDiscoverable, IDoomable
{
    ...
    public static void RpcTransport(PlayerControl transporter, byte player1, byte player2)
    {
        ...
        if (t1.TryCast<DeadBody>()) PreCheckHitman(t1.TryCast<DeadBody>()!);
        if (t2.TryCast<DeadBody>()) PreCheckHitman(t2.TryCast<DeadBody>()!);

        var positions = GetAdjustedPositions(t1, t2);

        Transport(t1, positions.Item2);
        Transport(t2, positions.Item1);
        var TosAbilityEvent = new TosAbilityEvent(AbilityType.TransporterTransport, transporter, t1, t2);
        ...
        if (play1.AmOwner || play2.AmOwner)
        {
            var notif1 = Helpers.CreateAndShowNotification(
                $"<b>{TownOfSushiColors.Transporter.ToTextColor()}You were transported!</color></b>", Color.white, spr: TosRoleIcons.Transporter.LoadAsset());
        ...
        void PreCheckUndertaker(DeadBody body)
        {
            ...
            var mods = ModifierUtils.GetActiveModifiers<UndertakerDragModifier>();
