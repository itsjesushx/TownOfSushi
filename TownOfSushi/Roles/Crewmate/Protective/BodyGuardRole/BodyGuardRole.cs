...
public sealed class BodyguardRole(IntPtr cppPtr) : CrewmateRole(cppPtr), ITOSRole, IWikiDiscoverable, IMysticClue
{
    ...
    {
        IntroSound = TownOfSushiAudio.WarlockCurse,
        Icon = TownOfSushiAssets.Bodyguard
    };
    ...
    public static void RpcBodyguardGuardMurder(PlayerControl bodyGuardPlayer, PlayerControl target, PlayerControl attacker)
    {
        ...
        if (attacker.AmOwner)
        {
            var notif = Helpers.CreateAndShowNotification(Utils.ColorString(TownOfSushiColors.Bodyguard,
                $"<b>{target.Data.PlayerName}, was protected by a {bodyGuardPlayer.Data.PlayerName} (The Bodyguard)! They died with you as well!</b>"),

                Color.white, spr: TownOfSushiAssets.Bodyguard.LoadAsset());
    ...
    public static void RpcBodyguardNotify(PlayerControl player, PlayerControl source, PlayerControl target)
    {
        ...
        if (player.AmOwner)
        {
            Coroutines.Start(Utils.CoFlash(TownOfSushiColors.Bodyguard));
        }
