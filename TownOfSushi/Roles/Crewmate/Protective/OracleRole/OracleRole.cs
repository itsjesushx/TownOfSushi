...
public sealed class OracleRole(IntPtr cppPtr) : CrewmateRole(cppPtr), ITOSRole, IWikiDiscoverable, IMysticClue
{
    ...
    {
        Icon = TownOfSushiAssets.Oracle,
        IntroSound = TownOfSushiAudio.GuardianAngelSound
    };
    ...
    [
        ...
        new("Confess",
            ...
            TownOfSushiAssets.ConfessSprite)
    ];
    ...
    public void ReportOnConfession()
    {
        ...
        Utils.AddFakeChat(confessing.Data, title, report, false, true);
    }
    ...
    public static void RpcOracleBlessNotify(PlayerControl oracle, PlayerControl source, PlayerControl target)
    {
        ...
        if (oracle.AmOwner)
        {
            Coroutines.Start(Utils.CoFlash(TownOfSushiColors.Oracle));
            var notif1 = Helpers.CreateAndShowNotification(Utils.ColorString(TownOfSushiColors.Oracle,
                $"<b>Your blessing has saved {target.Data.PlayerName} from getting guessed!</b>"),
                Color.white, spr: TownOfSushiAssets.Oracle.LoadAsset());
