...
public sealed class DetectiveRole(IntPtr cppPtr) : CrewmateRole(cppPtr), ITownOfSushiRole, IWikiDiscoverable, IDoomable
{
    ...
    public void ExaminePlayer(PlayerControl player)
    {
        if (InvestigatedPlayers.Contains(player.PlayerId))
        {
            ...
            var notif1 = Helpers.CreateAndShowNotification($"<b>{TownOfSushiColors.Detective.ToTextColor()}{player.Data.PlayerName} was at the scene of {deadPlayer.Data.PlayerName}'s death!\nThey might be the killer or a witness.</b></color>", Color.white, new Vector3(0f, 1f, -20f), spr: TosRoleIcons.Detective.LoadAsset());
            notif1.Text.SetOutlineThickness(0.35f);
        }
        else
        {
            Coroutines.Start(MiscUtils.CoFlash(Color.green));
            var notif1 = Helpers.CreateAndShowNotification($"<b>{TownOfSushiColors.Detective.ToTextColor()}{player.Data.PlayerName} was not at the scene of the crime.</b></color>", Color.white, new Vector3(0f, 1f, -20f), spr: TosRoleIcons.Detective.LoadAsset());
            notif1.Text.SetOutlineThickness(0.35f);
        }
    }
