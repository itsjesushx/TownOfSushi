...
public sealed class InformantRole(IntPtr cppPtr) : CrewmateRole(cppPtr), ITOSRole, IWikiDiscoverable, IMysticClue
{
    ...
    {
        Icon = TownOfSushiAssets.Informant,
        IntroSound = TownOfSushiAudio.ToppatIntroSound
    };
        ...
        if (completedTasks == realTasks.Count && CanSeeRealKiller)
        {
            if (Player.AmOwner)
            {
                ...
                var notif1 = Helpers.CreateAndShowNotification(
                    ...
                    spr: TownOfSushiAssets.Informant.LoadAsset());

                notif1.AdjustNotification();
            }
            else if (IsTargetOfInformant(PlayerControl.LocalPlayer) && CanSeeRealKiller)
            {
                Coroutines.Start(Utils.CoFlash(TownOfSushiColors.Informant, alpha: 0.5f));
                var text = "The Informant can temporarily know what you are now!";

                var notif1 = Helpers.CreateAndShowNotification(
                    $"<b>{TownOfSushiColors.Informant.ToTextColor()}{text}</color></b>", Color.white,
                    new Vector3(0f, 1f, -20f), spr: TownOfSushiAssets.Informant.LoadAsset());
        ...
        if (revealedTarget != null && player.AmOwner)
        {
            ...
            if (!snitchRole._snitchArrows.ContainsKey(revealedTarget.PlayerId))
            {
                snitchRole._snitchArrows.Add(revealedTarget.PlayerId, 
                    Utils.CreateArrow(revealedTarget.transform, TownOfSushiColors.Impostor));
            }
        }
    }
    ...
    private void CreateRevealingArrow()
    {
        ...
        InformantRevealArrow = Utils.CreateArrow(Player.transform, TownOfSushiColors.Informant);
    }
    ...
    private void CreateInformantArrows()
    {
        ...
        var imps = Helpers.GetAlivePlayers().Where(plr => plr.Data.Role.IsImpostor);
        imps.ToList().ForEach(imp =>
        {
            _snitchArrows.Add(imp.PlayerId, Utils.CreateArrow(imp.transform, TownOfSushiColors.Impostor));
            PlayerNameColor.Set(imp);
            imp.AddModifier<InformantTargetRevealModifier>();
        });

        var neutrals = Utils.GetRoles(RoleAlignment.NeutralKilling)
        ...
        neutrals.ToList().ForEach(neutral =>
        {
            ...
            {
                _snitchArrows.Add(neutral.Player.PlayerId, Utils.CreateArrow(neutral.Player.transform, TownOfSushiColors.Neutral));
