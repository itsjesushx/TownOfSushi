...
public sealed class JailedModifier(byte jailorId) : BaseModifier
{
    ...
    public override void OnMeetingStart()
    {
        ...
        if (Player.AmOwner)
        {
            ...
            if (PlayerControl.LocalPlayer.Is(Factions.Crewmate))
            {
                text =
            ...
            Utils.AddFakeChat(PlayerControl.LocalPlayer.Data, title, text, false, true);

            var notif1 = Helpers.CreateAndShowNotification(Utils.ColorString(TownOfSushiColors.Jailor,
                $"<b>{text}</b>"), Color.white,
                spr: TownOfSushiAssets.Jailor.LoadAsset());
    ...
    private void GenCell(PlayerVoteArea voteArea)
    {
        ...
        cellRenderer.sprite = TownOfSushiAssets.InJailSprite.LoadAsset();
