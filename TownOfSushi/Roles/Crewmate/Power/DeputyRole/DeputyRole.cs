...
public sealed class DeputyRole(IntPtr cppPtr) : CrewmateRole(cppPtr), ITOSCrewRole, IWikiDiscoverable, IMysticClue
{
    ...
    public override void Initialize(PlayerControl player)
    {
        ...
        if (Player.AmOwner)
        {
            meetingMenu = new MeetingMenu(
                ...
                TownOfSushiAssets.ShootMeetingSprite,
                null!,
                IsExempt)
            {
    ...
    public void ClickGuess(PlayerVoteArea voteArea, MeetingHud __)
    {
        ...
        if (!target.Is(Factions.Crewmate) && !target.IsProtected())
        {
        ...
        else
        {
            ...
            if (MissedShots == 1)
            {
                ...
                Utils.AddFakeChat(PlayerControl.LocalPlayer.Data, title,
                    "You missed your shot! You lost half your vision. Next time you miss you will die.", false, true);

                var notif1 = Helpers.CreateAndShowNotification(Utils.ColorString(TownOfSushiColors.Deputy,
                    $"<b>You missed your shot! You lost half your vision. Next time you miss you will die.</b>"),
                    Color.white, new Vector3(0f, 1f, -20f), spr: TownOfSushiAssets.Deputy.LoadAsset());
            ...
            {
                // Second miss: suicide
                var notif2 = Helpers.CreateAndShowNotification(Utils.ColorString(TownOfSushiColors.Deputy,
                    $"<b>You missed again... and paid the price for it.</b>"),
                    Color.white, new Vector3(0f, 1f, -20f), spr: TownOfSushiAssets.Deputy.LoadAsset());
