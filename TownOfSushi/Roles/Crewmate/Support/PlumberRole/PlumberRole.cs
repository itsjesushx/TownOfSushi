...
public sealed class PlumberRole(IntPtr cppPtr) : CrewmateRole(cppPtr), ITOSRole, IWikiDiscoverable, IMysticClue
{
    ...
    public CustomRoleConfiguration Configuration => new(this)
    {
        ...
        Icon = TownOfSushiAssets.Plumber
    };
        ...
        if (VentsBlocked.Count > 0 || FutureBlocks.Count > 0)
        {
            ...
            if (VentsBlocked.Count > 0)
            {
                foreach (var ventPair in VentsBlocked)
                {
                    ...
                    stringB.Append(TownOfSushiPlugin.Culture,
                        $"\n{Utils.GetRoomName(vent.transform.position)} Vent{text2}");
                }
            }
            if (FutureBlocks.Count > 0)
            {
                foreach (var ventId in FutureBlocks)
                {
                    ...
                    stringB.Append(TownOfSushiPlugin.Culture,
                        $"\n<color=#BFBFBF>{Utils.GetRoomName(vent.transform.position)} Vent: Preparing...</color>");
                }
            }
        }
        ...
        foreach (var ventId in FutureBlocks)
        {
            ...
            switch (ShipStatus.Instance.Type)
            {
                case ShipStatus.MapType.Fungle:
                    render.sprite = TownOfSushiAssets.BarricadeFungleSprite.LoadAsset();
                    barricade.transform.localPosition = new Vector3(0.03f, -0.107f, -0.001f);
                    break;
                case ShipStatus.MapType.Pb:
                    render.sprite = TownOfSushiAssets.BarricadeVentSprite.LoadAsset();
    ...
    public static void RpcPlumberFlush(PlayerControl player)
    {
        ...
        if (PlayerControl.LocalPlayer.inVent)
        {
            ...
            Coroutines.Start(Utils.CoFlash(TownOfSushiColors.Plumber));
        }
