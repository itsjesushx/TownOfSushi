    ...
    public override void OnActivate()
    {
        ...
        switch (OptionGroupSingleton<MediumOptions>.Instance.ArrowVisibility)
        {
            case MediumVisibility.Both:
                ...
                _arrow = Utils.CreateArrow(ownerTransform, TownOfSushiColors.Medium);
                break;

            case MediumVisibility.ShowMedium when Player.AmOwner:
                _arrow = Utils.CreateArrow(_mediumPlayer.transform, TownOfSushiColors.Medium);
                break;

            case MediumVisibility.ShowMediate when _mediumPlayer.AmOwner:
                _arrow = Utils.CreateArrow(Player.transform, TownOfSushiColors.Medium);
                break;
        }
        ...
        Coroutines.Start(Utils.CoFlash(TownOfSushiColors.Medium, alpha: 0.5f));
    }
