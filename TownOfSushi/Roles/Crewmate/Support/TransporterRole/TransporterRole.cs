...
public sealed class TransporterRole(IntPtr cppPtr) : CrewmateRole(cppPtr), ITOSRole, IWikiDiscoverable, IMysticClue
{
    [HideFromIl2Cpp]
    ...
    {
        Icon = TownOfSushiAssets.Transporter,
        IntroSound = TownOfSushiAudio.TimeLordIntroSound
    };
    ...
    public static void RpcTransport(PlayerControl transporter, byte player1, byte player2)
    {
        ...
        var play1 = Utils.PlayerById(player1)!;
        var play2 = Utils.PlayerById(player2)!;
        ...
        {
            var notif1 = Helpers.CreateAndShowNotification(Utils.ColorString(TownOfSushiColors.Transporter,
                $"<b>You were transported!</color></b>"), Color.white,
                spr: TownOfSushiAssets.Transporter.LoadAsset());
            notif1.AdjustNotification();
            Coroutines.Start(Utils.CoFlash(TownOfSushiColors.Transporter));
    ...
    public static void Transport(MonoBehaviour mono, Vector3 position)
    {
        ...
        if (mono.TryCast<DeadBody>() is { } deadBody &&
            Utils.PlayerById(deadBody.ParentId)?.HasModifier<LazyModifier>() == true)
        {
            return;
        }
        ...
        {
            Utils.SnapPlayerCamera(PlayerControl.LocalPlayer);
        }
    }
    ...
    public static void RpcMarkLocation(PlayerControl player, Vector2 pos)
    {
        ...
        henry.EscapeMark = AnimStore.SpawnAnimAtPlayer(player, TownOfSushiAssets.EscapistMarkPrefab.LoadAsset());
