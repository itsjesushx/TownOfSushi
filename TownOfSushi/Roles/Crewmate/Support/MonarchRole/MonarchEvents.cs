...
public static class MonarchEvents
{
    ...
    public static void BeforeMurderEventHandler(BeforeMurderEvent @event)
    {
        ...
        if (source.HasModifier<MonarchKnightedModifier>() && target.Data.Role is MonarchRole)
        {
            ...
            if (source.AmOwner)
            {
                Coroutines.Start(Utils.CoFlash(TownOfSushiColors.Monarch, PlaySound: true));
                var notif1 = Helpers.CreateAndShowNotification(Utils.ColorString(TownOfSushiColors.Monarch,
                $"<b>You cannot kill {target.Data.PlayerName} because they are the Monarch who knighted you.</color></b>"),
                Color.white, spr: TownOfSushiAssets.KnightSprite.LoadAsset());
            ...
            {
                Coroutines.Start(Utils.CoFlash(TownOfSushiColors.Monarch, PlaySound: true));
                var notif2 = Helpers.CreateAndShowNotification(Utils.ColorString(TownOfSushiColors.Monarch,
