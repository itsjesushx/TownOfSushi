...
public sealed class ImitatorCacheModifier : BaseModifier, ICachedRole
{
    ...
    public bool Visible => Player.AmOwner || PlayerControl.LocalPlayer.HasDied() || LawyerRole.LawyerSeesRoleVisibilityFlag(Player) ||
    ...
    public override void OnActivate()
    {
        ...
        if (Player.AmOwner)
        {
            _meetingMenu = new MeetingMenu(
                ...
                TownOfSushiAssets.ImitateSelectSprite,
                TownOfSushiAssets.ImitateDeselectSprite,
                IsExempt,
                Color.white)
            {
    ...
    {
        if (!Player.Is(Factions.Crewmate))
        {
    ...
    private bool IsExempt(PlayerVoteArea voteArea)
    {
        ...
        if (player != null && player.Object.GetRoleWhenAlive() is ICrewVariant neutVariant &&
            player.Object.Is(Factions.Neutral) && opts.ImitateNeutrals &&
            Utils.GetPotentialRoles().Contains(neutVariant.CrewVariant))
        {
        ...
        if (player != null && player.Object.GetRoleWhenAlive() is ICrewVariant impVariant &&
            ...
            Utils.GetPotentialRoles().Contains(impVariant.CrewVariant))
        {
            return voteArea.TargetPlayerId == Player.PlayerId || Player.Data.IsDead || !voteArea!.AmDead;
        }


        if (player != null && !player.Object.Is(Factions.Crewmate))
        {
            return true;
        }
