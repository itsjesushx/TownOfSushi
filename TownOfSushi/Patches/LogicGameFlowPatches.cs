...
public static class LogicGameFlowPatches
{
    ...
    public static bool CheckEndCriteriaPatch(LogicGameFlowNormal __instance)
    {
        ...
        if (CustomRoleUtils.GetActiveRolesOfTeam(ModdedRoleTeams.Custom)
                .OrderBy(x => (x as ITOSRole)!.RoleAlignment)
                .FirstOrDefault(x => x is ITOSRole role && role.WinConditionMet()) is { } winner)
        {
            Logger<TownOfSushiPlugin>.Message($"Game Over");
        ...
        if (Utils.NKillersAliveCount > 0 ||
            (Utils.ImpAliveCount > 0 && Utils.CrewKillersAliveCount > 0))
        {
            return false;
        }
