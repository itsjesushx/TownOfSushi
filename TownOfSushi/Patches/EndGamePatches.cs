...
public static class EndGamePatches
{
    public static void BuildEndGameData()
    {
        ...
        foreach (var playerControl in PlayerControl.AllPlayerControls)
        {
            ...
            if (lastRole is ITOSRole touRole)
            {
            ...
            {
                var modColor = Utils.GetRoleColour(modifierName.Replace(" ", string.Empty));
            ...
            if (killedPlayers > 0 && !playerControl.Is(Factions.Crewmate) && !playerControl.Is(RoleAlignment.NeutralEvil))
            {
                playerRoleString.Append(" | " + Utils.ColorString(TownOfSushiColors.Impostor, $"Kills: {killedPlayers}"));
            }

            if (GameHistory.PlayerStats.TryGetValue(playerControl.PlayerId, out var stats))
            {
                if (killedPlayers > 0 && playerControl.Is(Factions.Crewmate) && stats.CorrectKills <= 0 &&
                ...
                {
                    playerRoleString.Append(" | " + Utils.ColorString(TownOfSushiColors.Impostor,
            ...
            if (alliance != null)
            {
                var modColor = Utils.GetRoleColour(alliance.ModifierName.Replace(" ", string.Empty));
    ...
    public static void BuildEndGameSummary(EndGameManager instance)
    {
        ...
        GameSummaryButton.sprite = TownOfSushiAssets.GameSummarySprite.LoadAsset();
