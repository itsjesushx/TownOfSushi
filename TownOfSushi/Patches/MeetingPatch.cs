    ...
    class MeetingHudPatch 
    {
        ...
        class MeetingHudVotingCompletedPatch 
        {
            static void Postfix(MeetingHud __instance, [HarmonyArgument(0)]byte[] states, [HarmonyArgument(1)]NetworkedPlayerInfo exiled, [HarmonyArgument(2)]bool tie)
            {
                ...
                if (exiled != null) 
                {
                    ...
                    Pursuer.notAckedExiled = (Pursuer.pursuer != null && Pursuer.pursuer.PlayerId == exiled.PlayerId) || (Lawyer.Player != null && Lawyer.target != null && Lawyer.target.PlayerId == exiled.PlayerId && Lawyer.target != Jester.Player && !Lawyer.isProsecutor);
        ...
        static void SwapperOnClick(int i, MeetingHud __instance) 
        {
            ...
            else if (selectedCount == 1) 
            {
                ...
                else 
                {
                    selections[i] = true;
                    renderer.color = Color.yellow;
                    meetingExtraButtonLabel.text = Helpers.ColorString(Color.yellow, "Confirm Swap");
                }
            } 
        ...
        static void SwapperConfirm(MeetingHud __instance) 
        {
            ...
            if (firstPlayer != null && secondPlayer != null) 
            {
                ...
                meetingExtraButtonLabel.text = Helpers.ColorString(Color.green, "Swapping!");
                Swapper.charges--;
                ...
                if (!PlayerControl.LocalPlayer.Data.IsDead && !Helpers.PlayerById((byte)__instance.playerStates[buttonTarget].TargetPlayerId).Data.IsDead) button.GetComponent<PassiveButton>().OnClick.AddListener((System.Action)(() => 
                {
                    ...
                    else 
                    {
                        ...
                        if (focusedTarget == Crusader.FortifiedPlayer) 
                        {
                            ...
                            MessageWriter murderAttemptWriter = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.FortifiedMurderAttempt, Hazel.SendOption.Reliable, -1);
                            AmongUsClient.Instance.FinishRpcImmediately(murderAttemptWriter);
                        ...
                        FastDestroyableSingleton<HudManager>.Instance.StartCoroutine(Effects.Lerp(0.4f, new Action<float>((x) => {
                            if (x == 1f) {
                                foreach (PlayerControl p in PlayerControl.AllPlayerControls) {
                                    ...
                                    if (Snitch.targets == Snitch.Targets.EvilPlayers && p.IsCrew()) continue;
