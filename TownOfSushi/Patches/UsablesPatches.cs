...
namespace TownOfSushi.Patches
{
    ...
    public static class VentUsePatch
    {
        public static bool Prefix(Vent __instance)
        {
            ...
            bool jesterFlag = PlayerControl.LocalPlayer.IsJester(out _) && !CustomGameOptions.JesterCanMoveInVents;
            bool canMoveInVents = PlayerControl.LocalPlayer != Spy.Player && !jesterFlag && !Trapper.playersOnMap.Contains(PlayerControl.LocalPlayer.PlayerId);
    ...
    class VentButtonSetTargetPatch
    {
        ...
        static void Postfix(VentButton __instance)
        {
            ...
            if (isSpecialVent)
            {
                __instance.graphic.sprite = Utils.GetSprite("TricksterVentButton");
            ...
            __instance.buttonLabelText.enabled = true;
                // make sure we have the right position for the text, because for some reason without this the text is behind the button sprite
                if (__instance.buttonLabelText != null)
                {
                    var labelTransform = __instance.buttonLabelText.transform;
                    labelTransform.localPosition = new Vector3(labelTransform.localPosition.x,
                    labelTransform.localPosition.y, -1f);
                }
            }
        }
    }
    ...
    public class MurderPlayer
    {
        ...
        public class MurderPlayerPatch
        {
            ...
            {
                Utils.CheckMurderAttemptAndKill(__instance, target, true);
                ...
                if (BountyHunter.Player != null && PlayerControl.LocalPlayer == BountyHunter.Player && __instance == BountyHunter.Player)
                {
                    ...
                    {
                        BountyHunter.Player.SetKillTimer(CustomGameOptions.BountyHunterReducedCooldown);
                        BountyHunter.bountyUpdateTimer = 0f; // Force bounty update
                    }
                    else
                        BountyHunter.Player.SetKillTimer(GameOptionsManager.Instance.currentNormalGameOptions.KillCooldown + CustomGameOptions.BountyHunterPunishmentTime);
                }

                // Cleaner Button Sync
                if (Janitor.Player != null && PlayerControl.LocalPlayer == Janitor.Player && __instance == Janitor.Player && CustomButtonLoader.JanitorCleanButton != null)
                    CustomButtonLoader.JanitorCleanButton.Timer = Janitor.Player.killTimer;

                // Witch Button Sync
                if (CustomGameOptions.WitchTriggerBothCooldowns && Witch.Player != null && PlayerControl.LocalPlayer == Witch.Player && __instance == Witch.Player && CustomButtonLoader.witchSpellButton != null)
                    CustomButtonLoader.witchSpellButton.Timer = CustomButtonLoader.witchSpellButton.MaxTimer;

                // Warlock Button Sync
                if (Warlock.Player != null && PlayerControl.LocalPlayer == Warlock.Player && __instance == Warlock.Player && CustomButtonLoader.warlockCurseButton != null)
                {
                    if (Warlock.Player.killTimer > CustomButtonLoader.warlockCurseButton.Timer)
                    {
                        CustomButtonLoader.warlockCurseButton.Timer = Warlock.Player.killTimer;
                    }
                }
                // Assassin Button Sync
                if (Assassin.Player != null && PlayerControl.LocalPlayer == Assassin.Player && __instance == Assassin.Player && CustomButtonLoader.AssassinButton != null)
                    CustomButtonLoader.AssassinButton.Timer = CustomButtonLoader.AssassinButton.MaxTimer;
                return false;
            }
        }
                    ...
                    else
                    {
                        ...
                        if (res == MurderAttemptResult.BlankKill)
                        {
                            ...
                            if (PlayerControl.LocalPlayer == Janitor.Player)
                                Janitor.Player.killTimer = CustomButtonLoader.JanitorCleanButton.Timer = CustomButtonLoader.JanitorCleanButton.MaxTimer;
                            else if (PlayerControl.LocalPlayer == Warlock.Player)
                                Warlock.Player.killTimer = CustomButtonLoader.warlockCurseButton.Timer = CustomButtonLoader.warlockCurseButton.MaxTimer;
                            else if (PlayerControl.LocalPlayer == Witch.Player)
                                Witch.Player.killTimer = CustomButtonLoader.witchSpellButton.Timer = CustomButtonLoader.witchSpellButton.MaxTimer;
                            else if (PlayerControl.LocalPlayer == Assassin.Player)
                                Assassin.Player.killTimer = CustomButtonLoader.AssassinButton.Timer = CustomButtonLoader.AssassinButton.MaxTimer;
                        }
                    }
    ...
    class EmergencyMinigameUpdatePatch
    {
        static void Postfix(EmergencyMinigame __instance)
        {
            ...
            if (PlayerControl.LocalPlayer.IsJester(out _) && !CustomGameOptions.JesterCanCallEmergency)
            {
                CanCallEmergency = false;
                statusText = "The Jester can't start an emergency meeting";
            }
            // Potentially deactivate emergency button for Lawyer
            if (Lawyer.Player != null && Lawyer.Player == PlayerControl.LocalPlayer && !CustomGameOptions.LawyerCanCallEmergency)
            {
                CanCallEmergency = false;
            ...
            if (Executioner.Player != null && Executioner.Player == PlayerControl.LocalPlayer && !CustomGameOptions.ExecutionerCanCallEmergency)
            {
                CanCallEmergency = false;
            ...
            if (CustomGameOptions.LimitAbilities && Utils.TwoPlayersAlive() && Tiebreaker.Player != null && Tiebreaker.Player.IsCrew())
            {
                CanCallEmergency = false;
            ...
            if (__instance.state == 1)
            {
                ...
                int teamRemaining = Mathf.Max(0, CustomGameOptions.MaxNumberOfMeetings - MapOptions.meetingsCount);
    ...
    {
        private static List<TextMeshPro> hackerTexts = new List<TextMeshPro>();
                                ...
                                if (collider2D.CompareTag("DeadBody") && __instance.includeDeadBodies)
                                {
                                    ...
                                    if (bodyComponent)
                                    {
                                        ...
                                        if (playerInfo != null)
                                        {
                                            ...
                                            if (CustomGameOptions.HackerOnlyColorType)
    ...
    class SurveillanceMinigamePatch
    {
        ...
        private static Sprite overlaySprite = Utils.LoadSprite("TownOfSushi.Resources.NightVisionOverlay.png", 350f);
        ...
        private static void nightVisionUpdate(SurveillanceMinigame SkeldCamsMinigame = null, PlanetSurveillanceMinigame SwitchCamsMinigame = null, FungleSurveillanceMinigame FungleCamMinigame = null)
        {
            ...
            bool ignoreNightVision = CustomGameOptions.CamsNoNightVisionIfImpVision && Utils.HasImpVision(GameData.Instance.GetPlayerById(PlayerControl.LocalPlayer.PlayerId)) || PlayerControl.LocalPlayer.Data.IsDead;
            bool nightVisionEnabled = CustomGameOptions.CamsNightVision;
