...
public static class DummyBehaviourPatches
{
    ...
    private static IEnumerator tosDummyMode(PlayerControl dummy)
    {
        ...
        var randomUniMod = MiscUtils.AllModifiers.Where(x => x is UniversalGameModifier tosGameMod && tosGameMod.IsModifierValidOn(dummy.Data.Role)).Random();
        if (randomUniMod != null) dummy.RpcAddModifier(randomUniMod.GetType());
        
        var randomTeamMod = MiscUtils.AllModifiers.Where(x => x is TosGameModifier tosGameMod && tosGameMod.IsModifierValidOn(dummy.Data.Role)).Random();
