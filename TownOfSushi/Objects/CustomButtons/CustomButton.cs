...
namespace TownOfSushi.Objects.CustomButtons
{
    public class CustomButton 
    {
        ...
        public static class ButtonPositions 
        {
            ...
            public static readonly Vector3 lowerRowFarLeft = new Vector3(-5f, -0.06f, 0);
        ...
        public static void ResetAllCooldowns() 
        {
            for (int i = 0; i < buttons.Count; i++)
            {
                try
                {
                    ...
                    buttons[i].GlitchTimer = buttons[i].MaxTimer;
                    buttons[i].Update();
                }
                catch (NullReferenceException)
                {
        ...
        public void Update()
        {
            ...
            if (showButtonText && buttonText != "")
            {
                ...
                var role = Role.GetRoleInfoForPlayer(localPlayer);
                if (role == null) return;
                foreach (Role Role in role)
                {
                    actionButton.buttonLabelText.SetOutlineColor(Role.Color);
                }
            }
