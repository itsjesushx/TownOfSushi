...
public class TownOfSushiLocalSettings(ConfigFile config) : LocalSettingsTab(config)
{
  ...
  {
    TabIcon = TownOfSushiAssets.SushiRollSprite,
  };
  ...
  [LocalizedLocalToggleSetting]
    public ConfigEntry<bool> SeparateChatBubbles { get; private set; } = config.Bind("Miscellaneous", "Separate Chat Bubbles", false);
