...
public sealed class SeerRevealButton : TownOfSushiRoleButton<SeerRole, PlayerControl>
{
    ...
    public static void RevealAlliance(PlayerControl target)
    {
        ...
        if (IsEvil(target))
        {
            ...
            var notif1 = Helpers.CreateAndShowNotification($"<b>{TownOfSushiColors.ImpSoft.ToTextColor()}You have revealed that {target.Data.PlayerName} is {possiblyGood} evil!</color></b>", Color.white, spr: TosRoleIcons.Seer.LoadAsset());
        ...
        else
        {
            ...
            var notif1 = Helpers.CreateAndShowNotification($"<b>{Palette.CrewmateBlue.ToTextColor()}You have revealed that {target.Data.PlayerName} is {possiblyGood} good!</color></b>", Color.white, spr: TosRoleIcons.Seer.LoadAsset());
            notif1.Text.SetOutlineThickness(0.35f);
            notif1.transform.localPosition = new Vector3(0f, 1f, -20f);
