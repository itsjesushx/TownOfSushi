    ...
    public static void AfterMurderEventHandler(AfterMurderEvent @event)
    {
        ...
        if (PlayerControl.LocalPlayer.Data.Role is InquisitorRole)
        {
            if (victim.HasModifier<InquisitorHereticModifier>() && !victim.AmOwner && !source.AmOwner)
            {
                ...
                var notif1 = Helpers.CreateAndShowNotification($"<b>{TownOfSushiColors.Inquisitor.ToTextColor()}A Heretic has perished!</b></color>", Color.white, spr: TosRoleIcons.Inquisitor.LoadAsset());
            ...
            else if (!victim.HasModifier<InquisitorHereticModifier>() && !victim.AmOwner && source.AmOwner)
            {
                ...
                var notif1 = Helpers.CreateAndShowNotification($"<b>{TownOfSushiColors.Inquisitor.ToTextColor()}{victim.Data.PlayerName} was not a heretic!\nYou can no longer vanquish players.</b></color>", Color.white, spr: TosRoleIcons.Inquisitor.LoadAsset());
                notif1.Text.SetOutlineThickness(0.35f);
                notif1.transform.localPosition = new Vector3(0f, 1f, -20f);
            }
            else if (victim.HasModifier<InquisitorHereticModifier>() && !victim.AmOwner && source.AmOwner)
            {
                ...
                var notif1 = Helpers.CreateAndShowNotification($"<b>{TownOfSushiColors.Inquisitor.ToTextColor()}{victim.Data.PlayerName} was a heretic!</b></color>", Color.white, spr: TosRoleIcons.Inquisitor.LoadAsset());
                notif1.Text.SetOutlineThickness(0.35f);
                notif1.transform.localPosition = new Vector3(0f, 1f, -20f);
            }
        }
        ...
        if (inquis != null && inquis.TargetsDead && !inquis.Player.HasDied())
        {
            if (inquis.Player.AmOwner)
            {
                ...
                var notif1 = Helpers.CreateAndShowNotification(
                    $"<b>You have successfully won as the {TownOfSushiColors.Inquisitor.ToTextColor()}Inquisitor</color>, as all Heretics have perished!</b>", Color.white, spr: TosRoleIcons.Inquisitor.LoadAsset());
            ...
            {
                var notif1 = Helpers.CreateAndShowNotification(
                    $"<b>The {TownOfSushiColors.Inquisitor.ToTextColor()}Inquisitor</color>, {inquis.Player.Data.PlayerName}, has successfully won, as all Heretics have perished!</b>", Color.white, spr: TosRoleIcons.Inquisitor.LoadAsset());
