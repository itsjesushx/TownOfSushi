...
using TownOfSushi.Patches.MapChanges.CustomPolus;
...
public static class TownOfSushiEventHandlers
{
    ...
    public static void RunModChecks()
    {
        ...
        if (modifiers.Count > 0)
        {
            ...
            modifierText.Append("<size=2><color=#FFFFFF>Modifiers: </color>");
    ...
    public static IEnumerator CoChangeModifierText(IntroCutscene cutscene)
    {
        ...
        if (PlayerControl.LocalPlayer.Data.Role is ITOSRole custom)
        {
    ...
    public static void StartMeetingEventHandler(StartMeetingEvent @event)
    {
        ...
        Utils.Meetingtime = GameOptionsManager.Instance.currentNormalGameOptions.DiscussionTime + GameOptionsManager.Instance.currentNormalGameOptions.VotingTime + 7f;
        ...
        if (exeButton.Show || jestButton.Show)
        {
    ...
    public static void PlayerCanUseEventHandler(PlayerCanUseEvent @event)
    {
        ...
        if (@event.IsVent)
        {
            if (PlayerControl.LocalPlayer.inVent && Utils.TwoPlayersAlive() && OptionGroupSingleton<GeneralOptions>.Instance.LimitAbilities)
            {
                PlayerControl.LocalPlayer.MyPhysics.RpcExitVent(Vent.currentVent.Id);
                PlayerControl.LocalPlayer.MyPhysics.ExitAllVents();
            }

            if (Utils.TwoPlayersAlive())
            {
                @event.Cancel();
            }
        }
    ...
    public static void HandleMeetingMurder(MeetingHud instance, PlayerControl source, PlayerControl target)
    {
        ...
        if (PlayerControl.LocalPlayer.AmOwner || target.AmOwner || PlayerControl.LocalPlayer.Data.IsDead)
        {
