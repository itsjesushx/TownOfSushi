...
public static class DiseasedEvents
{
    ...
    public static void AfterMurderEventHandler(AfterMurderEvent @event)
    {
        ...
        if (!target.HasModifier<DiseasedModifier>() || target == source || MeetingHud.Instance) return;

        var cdMultiplier = OptionGroupSingleton<DiseasedOptions>.Instance.CooldownMultiplier;
        if (source.AmOwner)
        {
            var notif1 = Helpers.CreateAndShowNotification(
                $"<b>{TownOfSushiColors.Diseased.ToTextColor()}{@event.Target.Data.PlayerName} was Diseased, causing your kill cooldown to multiply by {Math.Round(cdMultiplier, 2)}.</color></b>", Color.white, spr: TosModifierIcons.Diseased.LoadAsset());
