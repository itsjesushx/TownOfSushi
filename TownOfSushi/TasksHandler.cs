            ...
            private static bool Prefix(GameData __instance) 
            {
                ...
                foreach (var playerInfo in GameData.Instance.AllPlayers.GetFastEnumerator())
                {
                    if (playerInfo.Object
                        ...
                        || playerInfo.PlayerId == Executioner.Player?.PlayerId // Tasks of the Executioner do not count
                        || playerInfo.PlayerId == Agent.Player?.PlayerId // Tasks of the Agent do not count
                        || playerInfo.Object.IsSurvivor(out _) // Tasks of the Survivor only count, if they are alive
                       )
                        continue;
