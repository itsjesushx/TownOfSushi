...
namespace TownOfSushi.Modules.BetterMaps.BetterSkeld
{
	...
	public static class BetterSkeld
	{
		...
		public static void AdjustSkeld()
		{
			if (IsObjectsFetched)
			{
                if (MapOptions.SkeldVentImprovements)
                {
                    MoveReactorVent();
                    MoveShieldsVent();
                    MoveBigYVent();
		...
		{
			if (ReactorVent == null)
				ReactorVent = UnityEngine.Object.FindObjectsOfType<Vent>().ToList().Find(vent => vent.gameObject.name == "ReactorVent");

			if (ShieldsVent == null)
				ShieldsVent = UnityEngine.Object.FindObjectsOfType<Vent>().ToList().Find(vent => vent.gameObject.name == "ShieldsVent");

			if (BigYVent == null)
				BigYVent = UnityEngine.Object.FindObjectsOfType<Vent>().ToList().Find(vent => vent.gameObject.name == "BigYVent");

			if (NavVentNorth == null)
				NavVentNorth = UnityEngine.Object.FindObjectsOfType<Vent>().ToList().Find(vent => vent.gameObject.name == "NavVentNorth");

			if (CafeVent == null)
				CafeVent = UnityEngine.Object.FindObjectsOfType<Vent>().ToList().Find(vent => vent.gameObject.name == "CafeVent");
