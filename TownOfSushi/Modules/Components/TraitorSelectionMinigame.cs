...
public sealed class TraitorSelectionMinigame(IntPtr cppPtr) : Minigame(cppPtr)
{
    ...
    {
        var gameObject = Instantiate(TosAssets.RoleSelectionGame.LoadAsset(), HudManager.Instance.transform);
    ...
    private void Begin()
    {
        ...
        {
            var teamName = role is ITownOfSushiRole tosRole
                ? tosRole.RoleAlignment.ToDisplayString()
                : role.TeamType.ToDisplayString();

            var roleName = role.NiceName;
            Sprite? roleImg = TosRoleIcons.RandomAny.LoadAsset();
        ...
        var randomCard = CreateCard("Random", "Random\nImpostor", TosRoleIcons.RandomImp.LoadAsset(), z, TownOfSushi.TownOfSushiColors.Impostor);
