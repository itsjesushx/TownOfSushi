...
public sealed class GuesserMenu(IntPtr cppPtr) : Minigame(cppPtr)
{
        ...
        Func<BaseModifier, bool>? modifierMatch = null, Action<BaseModifier>? modifierClickHandler = null)
    {
        ...
        var roles = Utils.GetPotentialRoles().Where(roleMatch).ToList();
        
        // This code adds Amne, Thief, Jester,  based on Executioner/Romantic Death settings
        var allRoles = Utils.AllRoles.Where(roleMatch).Where(x => x is IGuessable && !roles.Contains(x)).ToList();
        ...
        {
            var modifiers = Utils.AllModifiers.Where(modifierMatch).OrderBy(x => x.ModifierName).ToList();
