...
public static class SoftWikiEntries
{
    ...
    public static void RegisterRoleEntry(RoleBehaviour role)
    {
        ...
        if (roleEntry.Key != null && roleEntry.Value != null)
        {
            ...
            entry.GetAdvancedDescription = $"{role.BlurbLong}{Utils.AppendOptionsText(entry.EntryType)}";
    ...
    public static void RegisterModifierEntry(BaseModifier modifier)
    {
        ...
        if (roleEntry.Key != null && roleEntry.Value != null)
        {
            ...
            entry.GetAdvancedDescription = $"{modifier.GetDescription()}{Utils.AppendOptionsText(entry.EntryType)}";
            entry.Icon = modifier.ModifierIcon?.LoadAsset() ?? TownOfSushiAssets.RandomAny.LoadAsset();
        }
    }
}