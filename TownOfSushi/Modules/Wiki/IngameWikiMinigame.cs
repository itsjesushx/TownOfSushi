...
public sealed class IngameWikiMinigame(nint cppPtr) : Minigame(cppPtr)
{
    ...
    private void Awake()
    {
        ...
        HomepageModifiersBtn.Value.GetComponentInChildren<TextMeshPro>().text = "Modifiers";
        ...
        HomepageRolesBtn.Value.GetComponentInChildren<TextMeshPro>().text = "Roles";
        ...
        SearchScreenBackBtn.Value.GetComponentInChildren<TextMeshPro>().text = "Back";
        SearchScreenBackBtn.Value.OnClick.AddListener((UnityAction)(() => { UpdatePage(WikiPage.Homepage); }));

        DetailScreenBackBtn.Value.GetComponentInChildren<TextMeshPro>().text = "Back";
        ...
        SearchTextbox.Value.transform.GetParent().GetChild(2).GetComponent<TextMeshPro>().text = "Search Here";
        ...
        switch (newPage)
        {
            default:
                ...
                if (activeMods.Count > 0 && HomepageModifiersBtn.Value.transform.GetChild(0).TryGetComponent<SpriteRenderer>(out var modIcon))
                {
                    ...
                    modIcon.sprite = activeMods.Random()!.ModifierIcon?.LoadAsset() ?? TownOfSushiAssets.Bait.LoadAsset();
                }
                ...
                if (aliveRole != null && HomepageRolesBtn.Value.transform.GetChild(0).TryGetComponent<SpriteRenderer>(out var roleIcon))
                {
                    ...
                    roleIcon.sprite = aliveRole.RoleIconSolid ?? TownOfSushiAssets.Warlock.LoadAsset();
                }
    ...
    private void LoadDetailScreen()
    {
        ...
        if (_selectedItem is ITOSRole tosRole)
        {
            DetailScreenItemName.Value.text =
                $"{tosRole.RoleName}\n<size=60%>{tosRole.RoleColor.ToTextColor()}{Utils.GetParsedRoleAlignment(tosRole.RoleAlignment)}</size></color>";
            DetailScreenIcon.Value.sprite = tosRole.Configuration.Icon != null
                ? tosRole.Configuration.Icon.LoadAsset()
                : TownOfSushiAssets.RandomAny.LoadAsset();
        }
    ...
    private void LoadSearchScreen()
    {
        ...
        SearchPageIcon.Value.sprite = _modifiersSelected
            ? TownOfSushiAssets.Bait.LoadAsset()
            : TownOfSushiAssets.Hexblade.LoadAsset();
        if (!SearchIcon)
        {
            ...
            renderer.sprite = TownOfSushiAssets.Seer.LoadAsset();
        ...
        if (_modifiersSelected)
        {
            var activeModifiers = PlayerControl.LocalPlayer.GetModifiers<GameModifier>()
                ...
                .Select(x => Utils.GetModifierTypeId(x));
            ...
            {
                SearchPageIcon.Value.sprite = activeMods.Random()!.ModifierIcon?.LoadAsset() ?? TownOfSushiAssets.Bait.LoadAsset();
            }

            var modifiers = Utils.AllModifiers
            ...
            foreach (var modifier in modifiers)
            {
                ...
                if (color == TownOfSushiColors.Impostor)
                {
                    switch (modifier)
                    {
                        case UniversalGameModifier uniMod:
                            color = Utils.GetRoleColour(uniMod.LocaleKey.Replace(" ", string.Empty));
                            break;
                        case TOSGameModifier tosMod:
                            color = Utils.GetRoleColour(tosMod.LocaleKey.Replace(" ", string.Empty));
                            break;
                        case AllianceGameModifier allyMod:
                            color = Utils.GetRoleColour(allyMod.LocaleKey.Replace(" ", string.Empty));
                            break;
                    }
                }
                ...
                var alignment = Utils.GetParsedModifierFaction(modifier);
                ...
                else if (modifier is TOSGameModifier tosMod2)
                {
                    amount = tosMod2.CustomAmount;
                    chance = tosMod2.CustomChance;
                }
        ...
        else
        {
            ...
            if (aliveRole != null)
            {
                SearchPageIcon.Value.sprite = aliveRole.RoleIconSolid ?? TownOfSushiAssets.Hexblade.LoadAsset();
            }

            var comparer = new RoleComparer(roleList);
            var allRoles = Utils.AllRoles.ToList();
           /* if (LocalSettingsTabSingleton<TownOfSushiLocalSettings>.Instance.VanillaWikiEntriesToggle.Value)
            ...
            foreach (var role in roles)
            {
                ...
                var teamName = Utils.GetParsedRoleAlignment(role);
                var roleImg = TownOfSushiAssets.RandomAny.LoadAsset();
                if (customRole != null)
                {
                    // Hides hidden roles from other mods, but keeps them visible for Pest
                    ...
                    {
                        roleImg = TownOfSushiAssets.RandomCrew.LoadAsset();
                    }
                    else if (customRole.Team == ModdedRoleTeams.Impostor)
                    {
                        roleImg = TownOfSushiAssets.RandomImp.LoadAsset();
                    }
                    else
                    {
                        roleImg = TownOfSushiAssets.RandomNeut.LoadAsset();
                    }
                }
                else
                {
    ...
    private Transform CreateNewItem(string itemName, Sprite? sprite, Color color)
    {
        ...
        icon.sprite = sprite != null ? sprite : TownOfSushiAssets.RandomAny.LoadAsset();
    ...
    {
        var gameObject = Instantiate(TownOfSushiAssets.WikiPrefab.LoadAsset(), HudManager.Instance.transform);
