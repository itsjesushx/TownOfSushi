...
using Hazel;
using System.Text;
using static TownOfSushi.Modules.CustomOptions.CustomOption;
...
namespace TownOfSushi.Modules.CustomOptions
{
    public class CustomOption
    {
        ...
        public static int pasteFromClipboard() 
        {
            ...
            try 
            {
                ...
                string Settings = settingsSplit[1];
                string vanillaSettingsSub = settingsSplit[2];
                optionsFine = DeserializeOptions(Convert.FromBase64String(Settings));
                ShareOptionSelections();
                if (TownOfSushi.Version > versionInfo && versionInfo < Version.Parse("1.0.0"))
                {
            ...
            return Convert.ToInt32(vanillaOptionsFine) + optionsFine;
        }
    }
    ...
    class LobbyViewSettingsPatch
    {
        ...
        public static void CreateCustomButton(LobbyViewSettingsPane __instance, int targetMenu, string buttonName, string buttonText, CustomOptionType optionType) 
        {
            ...
            var SettingsButton = GameObject.Find(buttonName);
            if (SettingsButton == null) {
                SettingsButton = GameObject.Instantiate(buttonTemplate, buttonTemplate.transform.parent);
                SettingsButton.transform.localPosition += Vector3.right * 1.75f * (targetMenu - 2);
                SettingsButton.name = buttonName;
                __instance.StartCoroutine(Effects.Lerp(2f, new Action<float>(p => { SettingsButton.transform.FindChild("FontPlacer").GetComponentInChildren<TextMeshPro>().text = buttonText; })));
                var SettingsPassiveButton = SettingsButton.GetComponent<PassiveButton>();
                SettingsPassiveButton.OnClick.RemoveAllListeners();
                SettingsPassiveButton.OnClick.AddListener((System.Action)(() => {
                    __instance.ChangeTab((StringNames)targetMenu);
                }));
                SettingsPassiveButton.OnMouseOut.RemoveAllListeners();
                SettingsPassiveButton.OnMouseOver.RemoveAllListeners();
                SettingsPassiveButton.SelectButton(false);
                currentButtons.Add(SettingsPassiveButton);
        ...
        private static Tuple<string, string> HandleSpecialOptionsView(CustomOption option, string defaultString, string defaultVal) 
        {
            ...
            if (option == CustomOptionHolder.neutralKillingRolesCountMin) 
            { 
        ...
        public static void CreateSettingTabs(LobbyViewSettingsPane __instance)
        {
            ...
            // create TSR settings
            CreateCustomButton(__instance, next++, "TheSushiSettings", "General Mod Settings", CustomOptionType.General);
            // Impostor
            CreateCustomButton(__instance, next++, "ImposSettings", "Impostor Roles", CustomOptionType.Impostor);
            // Neutral
    ...
    class GameOptionsMenuStartPatch 
    {
        ...
        public static void Postfix(GameSettingMenu __instance) 
        {
            ...
            bgrenderer.sprite = Utils.LoadSprite("TownOfSushi.Resources.CopyPasteBG.png", 175f);
            ...
            copyButtonRenderer.sprite = Utils.LoadSprite("TownOfSushi.Resources.Copy.png", 100f);
            copyButton.transform.GetChild(1).transform.localPosition = Vector3.zero;
            copyButtonActiveRenderer.sprite = Utils.LoadSprite("TownOfSushi.Resources.CopyActive.png", 100f);
            ...
            pasteButtonRenderer.sprite = Utils.LoadSprite("TownOfSushi.Resources.Paste.png", 100f);
            pasteButtonActiveRenderer.sprite = Utils.LoadSprite("TownOfSushi.Resources.PasteActive.png", 100f);
        ...
        public static void CreateCustomButton(GameSettingMenu __instance, int targetMenu, string buttonName, string buttonText) 
        {
            ...
            var SettingsButton = GameObject.Find(buttonName);
            if (SettingsButton == null) {
                SettingsButton = GameObject.Instantiate(buttonTemplate, leftPanel.transform);
                SettingsButton.transform.localPosition += Vector3.up * 0.5f * (targetMenu - 2);
                SettingsButton.name = buttonName;
                __instance.StartCoroutine(Effects.Lerp(2f, new Action<float>(p => { SettingsButton.transform.FindChild("FontPlacer").GetComponentInChildren<TextMeshPro>().text = buttonText; })));
                var SettingsPassiveButton = SettingsButton.GetComponent<PassiveButton>();
                SettingsPassiveButton.OnClick.RemoveAllListeners();
        ...
        public static void CreateGameOptionsMenu(GameSettingMenu __instance, CustomOptionType optionType, string settingName) 
        {
            ...
            var SettingsTab = GameObject.Instantiate(tabTemplate, tabTemplate.transform.parent);
            SettingsTab.name = settingName;
                
            var SettingsGOM = SettingsTab.GetComponent<GameOptionsMenu>();

            UpdateGameOptionsMenu(optionType, SettingsGOM);

            currentTabs.Add(SettingsTab);
            SettingsTab.SetActive(false);
            currentGOMs.Add((byte)optionType, SettingsGOM);
        }

        public static void UpdateGameOptionsMenu(CustomOptionType optionType, GameOptionsMenu SettingsGOM) 
        {
            foreach (var child in SettingsGOM.Children) 
            {
                child.Destroy();
            }
            SettingsGOM.scrollBar.transform.FindChild("SliderInner").DestroyChildren();
            SettingsGOM.Children.Clear();
            var relevantOptions = options.Where(x => x.type == optionType).ToList();
            CreateSettings(SettingsGOM, relevantOptions);
        }
        ...
        private static void CreateSettingTabs(GameSettingMenu __instance) 
        {
            ...
            // Mod Settings
            CreateCustomButton(__instance, next++, "TheSushiSettings", "General Mod Settings");
        ...
        private static string BuildOptionsOfType(CustomOptionType type, bool headerOnly) 
        {
            ...
            foreach (CustomOption option in options) 
            {
                if (option.parent != null)
                {
                    ...
                    sb.AppendLine(Utils.ColorString(c, $"{option.name}: {option.selections[option.selection].ToString()}{option.Format}"));
                } 
                else 
                {
                    if (option == CustomOptionHolder.crewmateRolesCountMin) 
            ...
            string hudString = counter != 0 && !hideExtras ? Utils.ColorString(DateTime.Now.Second % 2 == 0 ? Color.white : Color.red, "(Use scroll wheel if necessary)\n\n") : "";
                ...
                switch (counter) 
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        hudString += "Page 2: General Mod Settings \n" + BuildOptionsOfType(CustomOptionType.General, false);
                        break;
                    case 2:
                        hudString += "Page 3: Roles, Modifiers & Ability Rates \n" + BuildRoleOptions();
                        break;
                    case 3:
    ...
    public static class GameOptionsNextPagePatch
    {
        public static void Postfix(KeyboardJoystick __instance)
        {
            ...
            {
                TownOfSushi.optionsPage = (TownOfSushi.optionsPage + 1) % 8;
            }
    ...
    public class HudManagerUpdate 
    {
        ...
        public static void Prefix2(HudManager __instance) 
        {
            ...
            {
                curBlock = blocks[i];
                if (Utils.lineCount(curBlock) + Utils.lineCount(curString) < 43) {
                    curString += curBlock + "\n\n";
                } else {
        ...
        {
            if (__instance.FullScreen == null || MapBehaviour.Instance && MapBehaviour.Instance.IsOpen || Utils.PreviousEndGameSummary.IsNullOrWhiteSpace()) return;
            if (settingsTMPs[0])
            {
                CloseSettings();
            }
        ...
        static PassiveButton RoleButton;
        static GameObject RoleButtonButtonObject;

        static PassiveButton SummaryButton;
        static GameObject SummaryButtonObject;

        static GameObject ZoomOutButtonObject;
        static PassiveButton ZoomOutButton;

        [HarmonyPostfix]
        public static void Postfix(HudManager __instance) 
        {
            if (AmongUsClient.Instance.GameState != InnerNet.InnerNetClient.GameStates.Started) return;
            if (!toggleSettingsButton || !GameSettingsButton)
            {
                // add a special button for settings viewing:
                GameSettingsButton = GameObject.Instantiate(__instance.MapButton.gameObject, __instance.MapButton.transform.parent);
                GameSettingsButton.transform.localPosition = __instance.MapButton.transform.localPosition + new Vector3(0, -1.25f, -500f);
                GameSettingsButton.name = "TOGGLESETTINGSBUTTON";
                SpriteRenderer renderer = GameSettingsButton.transform.Find("Inactive").GetComponent<SpriteRenderer>();
                SpriteRenderer rendererActive = GameSettingsButton.transform.Find("Active").GetComponent<SpriteRenderer>();
                GameSettingsButton.transform.Find("Background").localPosition = Vector3.zero;
                renderer.sprite = Utils.LoadSprite("TownOfSushi.Resources.Settings_Button.png", 100f);
                rendererActive.sprite = Utils.LoadSprite("TownOfSushi.Resources.Settings_ButtonActive.png", 100);
                toggleSettingsButton = GameSettingsButton.GetComponent<PassiveButton>();
                toggleSettingsButton.OnClick.RemoveAllListeners();
                toggleSettingsButton.OnClick.AddListener((Action)(() => ToggleSettings(__instance)));
            }
            GameSettingsButton.SetActive(__instance.MapButton.gameObject.active && !(MapBehaviour.Instance && MapBehaviour.Instance.IsOpen) && GameOptionsManager.Instance.currentGameOptions.GameMode != GameModes.HideNSeek);
            GameSettingsButton.transform.localPosition = __instance.MapButton.transform.localPosition + new Vector3(0, -0.8f, -500f);

            if (!RoleButton || !RoleButtonButtonObject) 
            {
                // add a special button for descriptions viewing:
                RoleButtonButtonObject = GameObject.Instantiate(__instance.MapButton.gameObject, __instance.MapButton.transform.parent);
                RoleButtonButtonObject.transform.localPosition = __instance.MapButton.transform.localPosition + new Vector3(0, -1.25f, -500f);
                RoleButtonButtonObject.name = "TOGGLEDESC";
                SpriteRenderer renderer = RoleButtonButtonObject.transform.Find("Inactive").GetComponent<SpriteRenderer>();
                SpriteRenderer rendererActive = RoleButtonButtonObject.transform.Find("Active").GetComponent<SpriteRenderer>();
                RoleButtonButtonObject.transform.Find("Background").localPosition = Vector3.zero;
                renderer.sprite = Utils.LoadSprite("TownOfSushi.Resources.InfoButton.png", 100f);
                rendererActive.sprite = Utils.LoadSprite("TownOfSushi.Resources.InfoButton_Active.png", 100);
                RoleButton = RoleButtonButtonObject.GetComponent<PassiveButton>();
                RoleButton.OnClick.RemoveAllListeners();
                RoleButton.OnClick.AddListener((Action)(() => Utils.ShowRole()));
            }
            RoleButtonButtonObject.SetActive(__instance.MapButton.gameObject.active && !(MapBehaviour.Instance && MapBehaviour.Instance.IsOpen) && GameOptionsManager.Instance.currentGameOptions.GameMode != GameModes.HideNSeek);
            RoleButtonButtonObject.transform.localPosition = HudManager.Instance.MapButton.transform.localPosition + new Vector3(-0.8f, -0.8f, -500f);

            if (!ZoomOutButton || !ZoomOutButtonObject) 
            {
                // add a special button for settings viewing:
                ZoomOutButtonObject = GameObject.Instantiate(__instance.MapButton.gameObject, __instance.MapButton.transform.parent);
                ZoomOutButtonObject.transform.localPosition = __instance.MapButton.transform.localPosition + new Vector3(-0.8f, -1.25f, -500f);
                ZoomOutButtonObject.name = "ZoomOutButton";
                SpriteRenderer tZrenderer = ZoomOutButtonObject.transform.Find("Inactive").GetComponent<SpriteRenderer>();
                SpriteRenderer tZArenderer = ZoomOutButtonObject.transform.Find("Active").GetComponent<SpriteRenderer>();
                ZoomOutButtonObject.transform.Find("Background").localPosition = Vector3.zero;
                tZrenderer.sprite = Utils.LoadSprite("TownOfSushi.Resources.Minus_Button.png", 100f);
                tZArenderer.sprite = Utils.LoadSprite("TownOfSushi.Resources.Minus_ButtonActive.png", 100);
                ZoomOutButton = ZoomOutButtonObject.GetComponent<PassiveButton>();
                ZoomOutButton.OnClick.RemoveAllListeners();
                ZoomOutButton.OnClick.AddListener((Action)(() => Utils.ToggleZoom()));
            }
            ...
            ZoomOutButtonObject.SetActive(zoomButtonActive);
            var posOffset = Utils.zoomOutStatus ? new Vector3(-1.27f, -7.92f, -52f) : new Vector3(0, -1.6f, -52f);
            ZoomOutButtonObject.transform.localPosition = HudManager.Instance.MapButton.transform.localPosition + posOffset;
        }
        ...
        public static void Postfix2(HudManager __instance) 
        {
            ...
            {
                if (SummaryButtonObject != null) 
                {
                    SummaryButtonObject.SetActive(false);
                    SummaryButtonObject.Destroy();
                    SummaryButton.Destroy();
                }
                return;
            }
            if (!SummaryButton || !SummaryButtonObject) 
            {
                SummaryButtonObject = GameObject.Instantiate(__instance.MapButton.gameObject, __instance.MapButton.transform.parent);
                SummaryButtonObject.transform.localPosition = __instance.MapButton.transform.localPosition + new Vector3(0, -1.25f, -500f);
                SummaryButtonObject.name = "TOGGLESUMMARYSBUTTON";
                SpriteRenderer renderer = SummaryButtonObject.transform.Find("Inactive").GetComponent<SpriteRenderer>();
                SpriteRenderer rendererActive = SummaryButtonObject.transform.Find("Active").GetComponent<SpriteRenderer>();
                SummaryButtonObject.transform.Find("Background").localPosition = Vector3.zero;
                renderer.sprite = Utils.LoadSprite("TownOfSushi.Resources.Endscreen.png", 100f);
                rendererActive.sprite = Utils.LoadSprite("TownOfSushi.Resources.EndscreenActive.png", 100f);
                SummaryButton = SummaryButtonObject.GetComponent<PassiveButton>();
                SummaryButton.OnClick.RemoveAllListeners();
                SummaryButton.OnClick.AddListener((Action)(() => ToggleSummary(__instance)));
            }
            SummaryButtonObject.SetActive(__instance.SettingsButton.gameObject.active && LobbyBehaviour.Instance && !Utils.PreviousEndGameSummary.IsNullOrWhiteSpace() && GameOptionsManager.Instance.currentGameOptions.GameMode != GameModes.HideNSeek 
                && AmongUsClient.Instance.GameState != InnerNet.InnerNetClient.GameStates.Started);
            SummaryButtonObject.transform.localPosition = __instance.SettingsButton.transform.localPosition + new Vector3(-1.45f, 0.03f, -500f);
        }
    }
}
