...
public sealed class Trap : IDisposable
{
    ...
    public void Update()
    {
        ...
        foreach (var player in PlayerControl.AllPlayerControls)
        {
            ...
            if (_players.ContainsKey(entry.PlayerId))
            {
                ...
                // Logger<TownOfSushiPlugin>.Error($"player with byte {entry.PlayerId} is logged with time {_players[entry.PlayerId]}");
                if (_players[entry.PlayerId] > MinAmountOfTimeInTrap && !_owner!.TrappedPlayers.Contains(role) &&
                    entry != _owner.Player)
                    // Logger<TownOfSushiPlugin>.Error($"Trap.Updated add role: {role.NiceName}");
                {
    ...
    public static void CreateTrap(TrapperRole player, Vector3 location)
    {
        ...
        trapPref.GetComponent<MeshRenderer>().material = AnimationMaterials.TrapMaterial.LoadAsset();
