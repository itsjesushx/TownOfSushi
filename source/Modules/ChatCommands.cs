    ...
    public static class ChatCommands
    {
        ...
        public static class PrivateJaileeChat
        {
            public static bool Prefix(ChatController __instance, [HarmonyArgument(0)] ref PlayerControl sourcePlayer, ref string chatText)
            {
                if (sourcePlayer == PlayerControl.LocalPlayer)
                {
                    ...
                    else if (chatText.ToLower().StartsWith("/r") || chatText.ToLower().StartsWith("/ m") || chatText.ToLower().StartsWith("/m") || chatText.ToLower().StartsWith("/ myrole"))
                    {
                        ...
                        else if (AmongUsClient.Instance.GameState == InnerNet.InnerNetClient.GameStates.Started) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer, "You do not have a role.");
                        else HUDManager().Chat.AddChat(PlayerControl.LocalPlayer, "Invalid Command.");
                        return false;
                    }
                    else if (chatText.ToLower().StartsWith("/md") || chatText.ToLower().StartsWith("/ md") || chatText.ToLower().StartsWith("/mymodifier") || chatText.ToLower().StartsWith("/ mymodifier"))
                    {
                        ...
                        else if (AmongUsClient.Instance.GameState == InnerNet.InnerNetClient.GameStates.Started) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer, "You do not have a modifier.");
                        else HUDManager().Chat.AddChat(PlayerControl.LocalPlayer, "Invalid Command.");
                        return false;
                    }
                    else if (chatText.ToLower().StartsWith("/ma") || chatText.ToLower().StartsWith("/ ma") || chatText.ToLower().StartsWith("/myability") || chatText.ToLower().StartsWith("/ myability"))
                    {
                        ...
                        else if (AmongUsClient.Instance.GameState == InnerNet.InnerNetClient.GameStates.Started) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer, "You do not have an ability.");
                        else HUDManager().Chat.AddChat(PlayerControl.LocalPlayer, "Invalid Command.");
                        return false;
                    }
                }
                ...
                {
                    if (sourcePlayer == PlayerControl.LocalPlayer) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer, "Invalid Command.");
                    return false;
                }
                ...
                if (PlayerControl.LocalPlayer.IsJailed() && Meeting()) return false;
                return true;
            }
            ...
            public static void AddRoleMessage(RoleEnum role)
            {
                ...
                if (role == RoleEnum.Crewmate) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer, "The Crewmate is a vanilla Crewmate.");
                if (role == RoleEnum.Impostor) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer, "The Impostor a vanilla Impostor.");
                if (role == RoleEnum.Engineer) HUDManager().Chat.AddChat(
                    PlayerControl.LocalPlayer, "The Engineer is a crewmate with the ability to vent and fix sabotages.");
                if (role == RoleEnum.Investigator) HUDManager().Chat.AddChat(
                    ...
                    if (role == RoleEnum.Romantic) HUDManager().Chat.AddChat(
                ...
                if (role == RoleEnum.Hunter) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer,
                ...
                if (role == RoleEnum.Arsonist) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer,
                ...
                if (role == RoleEnum.Executioner) HUDManager().Chat.AddChat(
                    PlayerControl.LocalPlayer, "The Executioner is a neutral evil role with the goal to vote out a specific player.");
                if (role == RoleEnum.Glitch) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer,
                ...
                if (role == RoleEnum.Jester) HUDManager().Chat.AddChat(
                    PlayerControl.LocalPlayer, "The Jester is a neutral evil role with the goal to be voted out.");
                if (role == RoleEnum.Poisoner) HUDManager().Chat.AddChat(
                ...
                if (role == RoleEnum.Witch) HUDManager().Chat.AddChat(
                ...
                if (role == RoleEnum.Vulture) HUDManager().Chat.AddChat(
                ...
                if (role == RoleEnum.Pestilence) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer,
                ...
                if (role == RoleEnum.SerialKiller) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer,
                ...
                if (role == RoleEnum.Detective) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer,
                ...
                if (role == RoleEnum.Escapist) HUDManager().Chat.AddChat(
                    PlayerControl.LocalPlayer, "The Escapist is an impostor who can mark a location and recall to it later.");
                if (role == RoleEnum.Imitator) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer,
                ...
                if (role == RoleEnum.Bomber) HUDManager().Chat.AddChat(
                ...
                if (role == RoleEnum.Hitman) HUDManager().Chat.AddChat(
                ...
                if (role == RoleEnum.Agent) HUDManager().Chat.AddChat(
                ...
                if (role == RoleEnum.Doomsayer) HUDManager().Chat.AddChat(
                    PlayerControl.LocalPlayer, "The Doomsayer is a neutral evil role with the goal to guess 3 other player's roles to win.");
                if (role == RoleEnum.Vampire) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer,
                ...
                if (role == RoleEnum.Warlock) HUDManager().Chat.AddChat(
                    PlayerControl.LocalPlayer, "The Warlock is an impostor who can charge their kill button to kill multiple people at once.");
                if (role == RoleEnum.Oracle) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer,
                ...
                if (role == RoleEnum.Venerer) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer,
                ...
                if (role == RoleEnum.Crusader) HUDManager().Chat.AddChat(
                ...
                if (role == RoleEnum.BountyHunter) HUDManager().Chat.AddChat(
                ...
                if (role == RoleEnum.Jailor) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer,
                ...
                if (role == RoleEnum.Deputy) HUDManager().Chat.AddChat(PlayerControl.LocalPlayer,
            ...
            {
                if (ability == AbilityEnum.Assassin) HUDManager().Chat.AddChat(
                ...
                if (ability == AbilityEnum.Spy) HUDManager().Chat.AddChat(
        ...
        public static class SetName
        {
            ...
            {
                if (PlayerControl.LocalPlayer.Is(RoleEnum.Jailor) && Meeting())
                {
                ...
                if (PlayerControl.LocalPlayer.IsJailed() && Meeting())
                {
                    if (JailorMessage)
                    {
