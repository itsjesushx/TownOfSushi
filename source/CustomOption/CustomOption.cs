...
namespace TownOfSushi.CustomOption 
{
    public class CustomOption
    {
        ...
        public static int pasteFromClipboard() 
        {
            ...
            try 
            {
                ...
                System.Version versionInfo = System.Version.Parse(settingsSplit[0]);
                ...
                ShareOptionSelections();
                if (TownOfSushi.Version > versionInfo && versionInfo < System.Version.Parse("2.7.0")) {
    ...
    class LobbyViewSettingsPatch
    {
        ...
        public static void DrawTab(LobbyViewSettingsPane __instance, CustomOptionType optionType) 
        {
            ...
            if ((int)optionType == 99) 
            {
                ...
                relevantOptions.AddRange(options.Where(x => x.type == CustomOptionType.NK && x.isHeader));
                relevantOptions.AddRange(options.Where(x => x.type == CustomOptionType.ModifierAbility && x.isHeader));
            }
                ...
                if (option.isHeader && (int)optionType != 99 || (int)optionType == 99 && curType != option.type) 
                {
                    ...
                    if ((int)optionType == 99)
                        categoryHeaderMasked.Title.text = new Dictionary<CustomOptionType, string>() { { CustomOptionType.Impostor, "Impostor Roles" }, { CustomOptionType.Neutral, "Neutral Roles" },
                            { CustomOptionType.Crewmate, "Crewmate Roles" }, { CustomOptionType.ModifierAbility, "Modifiers" }, { CustomOptionType.NK, "Neutral Killing Roles" }, }[curType];
        ...
        public static void CreateSettingTabs(LobbyViewSettingsPane __instance) 
        {
            ...
            int next = 3;
                ...
                // Neutral Killer
                createCustomButton(__instance, next++, "NKSettings", "Neutral Killing Roles", CustomOptionType.NK);
                // Crew
    ...
    class GameOptionsMenuStartPatch 
    {
        ...
        private static void CreateSettings(GameOptionsMenu menu, List<CustomOption> options) 
        {
            ...
            foreach (CustomOption option in options) 
            {
                ...
                if (option.isHeader && option.Heading == "" && (option.type == CustomOptionType.Neutral || option.type == CustomOptionType.Crewmate || option.type == CustomOptionType.NK || option.type == CustomOptionType.Impostor || option.type == CustomOptionType.ModifierAbility)) 
                {
        ...
        private static void CreateSettingTabs(GameSettingMenu __instance) 
        {
            ...
            CreateGameOptionsMenu(__instance, CustomOptionType.NK, "NKSettings");
            // IMp
    ...
    class GameOptionsDataPatch
    {
        private static string BuildRoleOptions() 
        {
            ...
            var neuts = BuildOptionsOfType(CustomOptionType.NK, true) + "\n";
        ...
        public static string BuildModifierExtras(CustomOption customOption) 
        {
            ...
            if (customOption == CustomOptionHolder.GuardianAngelOn) 
            {
                return $" (Evil Target: {CustomOptionHolder.EvilTargetPercent.GetSelection() * 10}%)";
            }
            return "";
        }
        ...
        private static string BuildOptionsOfType(CustomOptionType type, bool headerOnly) 
        {
            ...
            foreach (var option in options) 
            {
                if (option.parent == null) 
                {
                    ...
                    if (type == CustomOptionType.ModifierAbility) line += BuildModifierExtras(option);
                    sb.AppendLine(line);
                }
            }
    ...
    public class HudManagerUpdate 
    {
        ...
        public static void Prefix3(HudManager __instance) {
            ...
            summaryTMP.text = PreviousEndGameSummary;
        ...
        {
            if (__instance.FullScreen == null || MapBehaviour.Instance && MapBehaviour.Instance.IsOpen || PreviousEndGameSummary.IsNullOrWhiteSpace()) return;
            if (settingsTMPs[0]) {
                CloseSettings();
            }
        ...
        public static void Postfix(HudManager __instance) 
        {
            ...
            if (!toggleRoleInfo || !toggleRoleInfoButtonObject) 
            {
                ...
                renderer.sprite = LoadSpriteFromResources("TownOfSushi.Resources.InfoButton.png", 100f);
                rendererActive.sprite = LoadSpriteFromResources("TownOfSushi.Resources.InfoButton_Active.png", 100);
                toggleRoleInfo = toggleRoleInfoButtonObject.GetComponent<PassiveButton>();
                toggleRoleInfo.OnClick.RemoveAllListeners();
                toggleRoleInfo.OnClick.AddListener((Action)(() => ShowPopUpPatch.ShowRoleInfo()));
            }
        ...
        public static void Postfix2(HudManager __instance) 
        {
            ...
            if (!toggleSummaryButton || !toggleSummaryButtonObject) 
            {
                ...
                renderer.sprite = LoadSpriteFromResources("TownOfSushi.Resources.Endscreen.png", 100f);
                rendererActive.sprite = LoadSpriteFromResources("TownOfSushi.Resources.EndscreenActive.png", 100f);
            ...
            toggleSummaryButtonObject.SetActive(__instance.SettingsButton.gameObject.active && LobbyBehaviour.Instance && !PreviousEndGameSummary.IsNullOrWhiteSpace() && GameOptionsManager.Instance.currentGameOptions.GameMode != GameModes.HideNSeek 
