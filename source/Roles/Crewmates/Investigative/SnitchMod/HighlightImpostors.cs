            ...
            foreach (var player in PlayerControl.AllPlayerControls)
                {
                    if (player.Data.IsImpostor())
                    {
                        ...
                        if (player.Is(AbilityEnum.Chameleon)) impcolour.a = Ability.GetAbility<Chameleon>(player).Opacity;
                        player.nameText().color = impcolour;
                    }
                    else if (player.Is(RoleAlignment.NeutralKilling))
                    {
                        var neutColour = Color.gray;
                        if (player.Is(AbilityEnum.Chameleon)) neutColour.a = Ability.GetAbility<Chameleon>(player).Opacity;
