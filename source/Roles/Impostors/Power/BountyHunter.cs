namespace TownOfSushi.Roles
{
    public class BountyHunter : Role
    {
        ...
        public PlayerControl AddBounty(PlayerControl toRemove = null)
        {
            ...
            if (Player.IsBeloved())targets = AllPlayers().Where(x => !x.Data.IsDead && !x.Data.Disconnected && !x.Is(Faction.Impostors) && !x.Is(RoleEnum.Romantic) && x != PlayerControl.LocalPlayer&& x != ShowRoundOneShield.FirstRoundShielded && x != toRemove).ToList();
            if (targets.Count == 0) targets = AllPlayers().Where(x => !x.Data.IsDead && !x.Data.Disconnected && !x.Is(Faction.Impostors) && x != PlayerControl.LocalPlayer&& x != ShowRoundOneShield.FirstRoundShielded && x != toRemove).ToList();
