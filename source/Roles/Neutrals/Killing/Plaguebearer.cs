namespace TownOfSushi.Roles
{
    public class Plaguebearer : Role
    {
        ...
        public bool CanTransform => AllPlayers().Count(x => x != null && !x.Data.IsDead && !x.Data.Disconnected) <= InfectedAlive;
    ...
    public class PlagueBodyReport
    {
        private static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] NetworkedPlayerInfo info)
        {
            ...
            foreach (var player in PlayerControl.AllPlayerControls)
            {
                ...
                {
                    if (LocalPlayer().IsInfected() || player.IsInfected())
                    {
                        foreach (var pb in GetRoles(RoleEnum.Plaguebearer)) ((Plaguebearer)pb).RpcSpreadInfection(LocalPlayer(), player);
                    }
                }
            }
        }
    ...
    public class PerformInfect
    {
        ...
        {
            var flag = LocalPlayer().Is(RoleEnum.Plaguebearer);
            if (!flag) return true;
            if (IsDead()) return false;
            if (!LocalPlayer().CanMove) return false;
            if (!__instance.isActiveAndEnabled || __instance.isCoolingDown) return false;
            var role = GetRole<Plaguebearer>(LocalPlayer());
    ...
    public static class HudInfect
    {
        public static void Postfix(HudManager __instance)
        {
            ...
            if (NullLocalPlayer()) return;
            if (NullLocalPlayerData()) return;
            if (!LocalPlayer().Is(RoleEnum.Plaguebearer)) return;
            var isDead = IsDead();
            ...
            var notInfected = AllPlayers().Where(
            ...
            if (role.CanTransform && (AllPlayers().Where(x => !x.Data.IsDead && !x.Data.Disconnected).ToList().Count > 1) && !isDead)
            {
                var transform = false;
                var alives = AllPlayers().Where(x => !x.Data.IsDead && !x.Data.Disconnected && x != LocalPlayer()).ToList();
                if (alives.Count <= 1)
                {
                ...
                {
                    role.TurnPestilence();
                    StartRPC(CustomRPC.TurnPestilence, LocalPlayer().PlayerId);
                }
            }
        }
    }
